...
RADAR_REF=a6aba2f2738ad97c01f3145943cb726afe4f8a14
RADAR_GENERATED_AT=2026-09-15T00:50:49.6715219+02:00
...
```text
node scripts/checks/check-top-heavy-files.mjs --ref=a6aba2f2738ad97c01f3145943cb726afe4f8a14 --top=25 --enforce
```

Mesures de la ref : `2223` fichiers mesurés, `92` en `REVIEW_REQUIRED`, `0` en
...
Ces décisions viennent de l'état architectural antérieur et sont simplement
exprimées avec la terminologie canonique. Elles ne sont pas réinventées par la
taille du snapshot.
...
| `apps/web/src/lib/auth/api-authorization-contract.ts` | 823 | 32507 | runtime | `COHESIVE_SINGLE_FILE` | Contrat d'autorisation cohésif ; la taille seule ne justifie pas une extraction. |
...
Les entrées suivantes dépassent le seuil de revue sur cette ref et ne sont pas
couvertes par une décision préexistante ci-dessus. Elles restent
`REVIEW_REQUIRED`: ce statut décrit un état d'audit, pas une prescription de
découpage.
...
| `apps/web/src/app/api/actions/[actionId]/route.ts` | 839 | 29675 | runtime | `REVIEW_REQUIRED` |
| `apps/web/src/app/api/actions/[actionId]/route.test.ts` | 807 | 26852 | test | `REVIEW_REQUIRED` |
| `apps/web/src/lib/actions/store.ts` | 797 | 25555 | runtime | `REVIEW_REQUIRED` |
...
| `apps/web/src/lib/actions/participation/group-participation.ts` | 706 | 21614 | runtime | `REVIEW_REQUIRED` |
...
Ce document ne refactore aucun candidat du radar. Les sorties de façades et
les décisions historiques restent des faits de contexte, à confirmer contre
la ref exacte avant toute modification future. Une nouvelle génération doit
remplacer `RADAR_REF`, `RADAR_GENERATED_AT` et les mesures avec le SHA réellement
scanné ; elle ne doit pas présenter un snapshot ancien comme l'état courant.
