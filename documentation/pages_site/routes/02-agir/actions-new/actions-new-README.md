...
- **Règle de modération** : un profil `admin`, `elu` ou `max` peut auto-valider sa propre action, mais un formulaire de groupe normal reste soumis au flux habituel et n'est jamais considéré comme une collecte validée tant que la déclaration finale n'est pas complétée. Une pré-action future valide est néanmoins persistée immédiatement et visible sur la carte ; cette visibilité ne vaut ni approbation métier ni éligibilité Impact.
