...
- **Formalités & autorisations locales** : le moteur versionné `action-formalities-qualification-v1` qualifie le périmètre Paris à partir de sources officielles vérifiées : AOT de la Ville pour une occupation du domaine public municipal avec installation ou emprise, déclaration auprès de la Préfecture de police pour les manifestations parisiennes entrant dans ce cadre, et formalité distincte du gestionnaire lorsque le lieu n'est pas municipal. Une simple cleanwalk sans installation n'est pas transformée en AOT par défaut ; les faits insuffisants produisent `unknown`, et les sorties conformes aux usages locaux restent une exception explicitement sourcée. Le panneau persiste séparément `formalitiesContext` (faits de qualification) et `formalitiesWorkflow` (trace de règle et état utilisateur `not_started | prepared | sent`). `sent` n'est produit que par une déclaration explicite de l'utilisateur avec une preuve minimale ; ouvrir un lien ou préparer un brouillon ne l'établit jamais. Le moteur ne fabrique aucune obligation, n'envoie aucune procédure et ne bloque pas encore la publication : le blocage final est réservé au lot suivant. Aucun canal email n'est considéré sans preuve officielle. Sources canoniques : [Ville de Paris](https://www.paris.fr/pages/evenements-dans-l-espace-public-33659), [Préfecture de police](https://www.prefecturedepolice.interieur.gouv.fr/demarches/manifestation-sur-la-voie-publique-ou-tout-espace-ouvert-au-public) et [Service-Public](https://www.service-public.gouv.fr/particuliers/vosdroits/F21899), vérifiées le 17/09/2026. Chaque pré-action porte en plus `administrativeRequirements.status`, initialisé à `pending` et distinct de `preparationState`.
