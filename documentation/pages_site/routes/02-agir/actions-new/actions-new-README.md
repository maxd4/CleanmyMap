...
- **Action principale attendue** : Utiliser le shell unique de création comme workflow guidé reprenable : `1 Itinéraire → 2 Paris → 3 Préparation → 4 Préformulaire`. Le `Formulaire` après action reste un onglet séparé.
...
- **Identité structurée** : `organizerType`, `organizerId` et `organizerName` forment le contrat canonique de l'organisateur. Les valeurs sont `spontaneous`, `company`, `association`, `student_association`, `collective` et `other` ; l'interface les présente comme `Action spontanée`, `Entreprise`, `Association`, `Association étudiante`, `Collectif` et `Autre`. `associationName` reste une façade de lecture/compatibilité pour les rapports, exports et anciennes actions, jamais la source de vérité de la nouvelle saisie.
- **Organisateur accessible** : le champ unique `Organisateur` est un combobox clavier et mobile. Après le choix de `Type de structure`, les suggestions sont limitées au même type ; une sélection conserve l'identifiant canonique. Une saisie libre est normalisée côté serveur, réutilise une entrée persistée exactement équivalente dans ce type ou crée une nouvelle entrée réutilisable ; l'approximation sert uniquement aux suggestions et ne fusionne jamais des structures. `Association` et `Association étudiante` restent deux types distincts. Pour `spontaneous`, le référent est attaché à l'action et n'est pas ajouté au catalogue des structures. Les anciennes valeurs `Entreprise - <nom>` restent lisibles, mais ne reconstruisent plus l'identité canonique.
- **Participants** : `volunteersCount` décrit le nombre de participants à l'action et ne doit pas être déduit de `organizerType`, qui décrit uniquement le cadre de l'organisateur. Pour `organizerType = spontaneous`, la catégorie statistique est calculée automatiquement depuis `volunteersCount` (`1 = Solo`, `2 = Duo`, `3 = Trio`, `4 = Quatuor`, `5 = Quintet`, `6 = Sextet`, puis le nom du groupe correspondant) ; aucun sélecteur Solo/Duo/Trio n'est affiché et cette catégorie n'est pas persistée comme une saisie indépendante. Pour une structure, `volunteersCount` alimente le KPI total Participants tandis que la catégorie reste celle de la structure (`Entreprise`, `Association`, `Association étudiante`, `Collectif` ou `Autres`) : il ne crée jamais une catégorie Quintet ou équivalente.
...
- **Workflow guidé** : chaque étape expose `À faire`, `En cours`, `Terminé` ou `À revoir`. L’utilisateur peut revenir en arrière ; l’orchestration locale ne conserve que l’étape et les statuts minimaux. Le planner route, le moteur météo, le workflow Paris et `ActionPreparationData` restent uniques.
...
- **Formalités & autorisations locales** : le moteur versionné `action-formalities-qualification-v1` est appelé par `/api/actions/[actionId]/formalities` et le panneau `Formalités` de la pré-action. Il qualifie le périmètre Paris à partir de sources officielles vérifiées : AOT de la Ville pour une occupation du domaine public municipal avec installation ou emprise, déclaration auprès de la Préfecture de police pour les manifestations parisiennes entrant dans ce cadre. Pour un lieu SNCF, État, HAROPA, privé ou autre gestionnaire non municipal, le moteur ne déclare pas `required` sans source officielle spécifique au gestionnaire : il recommande d'identifier et de confirmer la formalité, le délai et le canal. Une simple cleanwalk insuffisamment caractérisée produit `unknown` et n'est pas transformée en AOT par défaut ; les sorties conformes aux usages locaux restent une exception explicitement sourcée. Le panneau affiche la qualification, la justification, l'autorité, le délai et le lien officiel lorsque disponibles. Il persiste séparément `formalitiesContext` (faits de qualification) et `formalitiesWorkflow` (trace de règle et état utilisateur `not_started | prepared | sent`). `sent` n'est produit que par une déclaration explicite de l'utilisateur avec une preuve minimale ; ouvrir un lien ou préparer un brouillon ne l'établit jamais. Le moteur ne fabrique aucune obligation, n'envoie aucune procédure et ne bloque pas une action. Aucun canal email n'est considéré sans preuve officielle. Sources canoniques : [Ville de Paris](https://www.paris.fr/pages/evenements-dans-l-espace-public-33659), [Préfecture de police](https://www.prefecturedepolice.interieur.gouv.fr/demarches/manifestation-sur-la-voie-publique-ou-tout-espace-ouvert-au-public) et [Service-Public](https://www.service-public.gouv.fr/particuliers/vosdroits/F21899), vérifiées le 17/09/2026. Chaque pré-action porte en plus `administrativeRequirements.status`, initialisé à `pending` et distinct de `preparationState`.
...
- **Deep-links** : `tab=before|after` conserve les deux onglets historiques. Le workflow guidé accepte `step=itineraire|paris|preparation|preformulaire` et conserve `actionId`, `from`, `fromEventId` et les autres paramètres utiles. `/actions/new?actionId=<id>` reprend la même action canonique ; dès qu’une `pre_action` existe, `actions.id`, `action_phase` et `preparation_data` font foi. `/sections/route` redirige vers l’étape itinéraire et `/sections/weather` vers l’orchestrateur météo/préparation. Les anciens `panel=...` restent des alias de compatibilité et ne confèrent aucune permission.
...
- **shell** : le stepper guidé présente une seule étape active, avec navigation précédente/suivante et reprise. Les contenus métier ne sont pas recalculés par l’orchestrateur.
...
- Le point d'entrée est le shell unique `Organiser une action`, dont le stepper orchestre les quatre étapes sans dupliquer route, météo, action ou formalités.
...
- Le parcours avant action propose un récapitulatif `Préformulaire prêt à publier` uniquement lorsque la qualification Paris ne bloque pas selon le contrat administratif ; aucune publication, messagerie ou recrutement n’est déclenché par ce workflow.
