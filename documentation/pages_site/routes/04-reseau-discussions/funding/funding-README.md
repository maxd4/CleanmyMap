...
- **Action principale attendue** : Consulter les deux affectations, les agrégats publics et l'état de disponibilité ; aucun paiement n'est déclenchable depuis l'UI.
- **Palette attendue** : rose / pink / fuchsia sur fond réseau sombre
- **Scope réel** : présentation publique des deux catégories et des agrégats nets issus des contributions historiques confirmées, avec garanties d'indépendance. Les agrégats restent publics ; aucun Checkout ni paiement n'est exposé par l'UI.
- **Terminée** : oui
...
Cette stratégie ne crée ni lien fournisseur, ni bouton, ni parcours ou route. Elle ne promet aucun reçu fiscal ni avantage fiscal. Stripe n'est actuellement pas une voie de contribution proposée aux utilisateurs depuis cette page.

## États à documenter

- **loading** : les totaux affichent `Chargement…` pendant la lecture de l'agrégat public.
- **error** : les totaux indiquent leur indisponibilité et proposent une nouvelle tentative ; les boutons `Non disponible` / `Unavailable` restent désactivés.
- **availability** : les deux cartes affichent un bouton désactivé libellé `Non disponible` en français ou `Unavailable` en anglais ; aucun choix de montant ni appel UI vers `/api/funding/checkout` n'est fourni.
