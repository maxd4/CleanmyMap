...
- `apps/web/supabase/migrations/20260926000002_chat_polls_all_channels.sql`
...
- **Sondages** : `message_kind = 'poll'` réutilise la même infrastructure dans `community`, `admin_elu`, `territory`, `action` et `dm`. Un sondage est une aide à l'arbitrage, pas une décision officielle ; son résultat ne vaut pas décision officielle. La question reste dans `app_messages.content` et les 2 à 6 options ordonnées vivent dans `chat_poll_options`, créées atomiquement avec le message et son contexte de canal. Les votes vivent dans `chat_poll_votes`, avec une ligne par `(message_id, user_id)` ; les lectures retournent uniquement des compteurs agrégés et le choix de l'utilisateur courant, jamais une liste nominative. En DM, la visibilité et le vote restent limités aux deux participants par les contrôles existants du canal.
- **Topics** : `topic_id` est nullable ; `NULL` conserve les messages legacy/non classés. Les topics persistés restent `relais_associatif`, `appel_aux_benevoles`, `demande_diffusion`, `besoin_ressources`, `coordination_secteur`, `mon_territoire`, `territoires_voisins`, `arbitrages`, `priorites`, `suivi_decisions` et `coordination_institutionnelle`. L'interface regroupe uniquement leur présentation : `Mobiliser & relayer` agrège les trois topics de mobilisation/diffusion, `Ressources` et `Coordination` restent chacun un topic, le territoire devient un fil piloté par la zone, et `Arbitrages & priorités` agrège `arbitrages` et `priorites`. Aucun topic libre n'est accepté par le client et aucune donnée n'est migrée ou supprimée.
...
- **Capture photo** : `Prendre une photo` utilise un input navigateur natif `accept="image/*"` avec `capture="environment"` lorsque le terminal le prend en charge. Sur desktop, le même input retombe sur la sélection d’une image et réutilise exactement le pipeline d’attachement existant. Les vidéos sont refusées par le contrat Chat, l’API et l’allowlist MIME Storage ; le message affiché est `Les vidéos ne sont pas prises en charge. Partagez plutôt une photo ou un lien vers la vidéo.`
- **Pièces jointes et contenu non fiable** : les nouveaux formats image Chat sont l’allowlist raster existante, PDF et documents autorisés ; SVG est explicitement exclu du sélecteur, de la validation fichier/MIME, de l’API et du bucket privé. Les anciens messages SVG restent lisibles si leur référence existe déjà. Une pièce jointe est affichée comme fichier ou image selon son type validé ; aucun document n’est exécuté, aucun aperçu distant n’est récupéré et aucune URL `javascript:`, `data:` ou protocole inconnu n’est accepté.
...
- **Finition accessibilité/UI** : le titre de page utilise `PageHeader` ; le shell reste sur une surface claire rose/pink, avec l’indigo réservé aux repères privés/relationnels. Les tabs primaires exposent une sémantique `tablist`/`tab`/`tabpanel`, les contrôles gardent un focus visible et les animations respectent `prefers-reduced-motion` ainsi que les modes `minimaliste` et `sobre`. Avant connexion, la surface reste floutée et propose un lien de connexion explicite ; les messages privés n’affichent jamais de parcours d’envoi sans connexion. Le statut réseau n’est affiché que lorsqu’il apporte une information utile.
