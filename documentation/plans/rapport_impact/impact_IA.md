...
- L’ordre de grandeur retenu pour l’assistance IA et le développement associé est d’environ **100 kWh**, **20 kgCO2e** et **100 L d’eau** pour **100 h** de travail assisté, avec une forte dépendance au mix électrique, aux services cloud et au volume réel de requêtes.
- Ces valeurs doivent être lues comme des ordres de grandeur, non comme une mesure instrumentée, et elles s’ajoutent aux coûts d’usage du site, des images, des builds, du stockage et des services tiers.
...
- Privilégier les modèles et les workflows les plus sobres pour les besoins courants.
- Encadrer les prompts, les usages et les validations humaines par une gouvernance explicite.
- Renforcer la cybersécurité, la limitation des accès et la transparence sur les choix techniques et les dépendances.

## Guide de lecture {.unnumbered}

- **Lecture rapide** : lire le [Résumé exécutif](#resume-executif) puis les [Recommandations prioritaires](#recommandations-prioritaires).
- **Lecture méthodologique** : lire la [Partie I](#partie-i-cadre-perimetre-et-methode).
- **Lecture environnementale** : lire la [Partie II](#partie-ii-empreinte-environnementale-et-materielle), puis les passages sur l’empreinte matérielle, le carbone, l’eau et l’ACV.
- **Lecture sociale et gouvernance** : lire la [Partie III](#partie-iii-analyse-sociale-humaine-et-politique-de-lia), la [Partie V](#partie-v-bilan-systemique-dette-compensation-et-limites) et la [Partie VI](#partie-vi-plan-de-reduction-des-impacts).
- **Lecture technique** : lire les [Annexes A](#annexe-a), [B](#annexe-b-methodologie-de-calcul-et-incertitudes) et [C](#annexe-c-faq), ainsi que les passages de la Partie I sur le périmètre technique et les hypothèses de dépôt.
...
- Backend/API : routes API Next.js
...
- Legacy : Python, SQLite, scripts, tests historiques et fichiers textes du développeur
...
L’indicateur central du rapport est l’IUR, pour Indice d’Utilité Réelle. Il se formule simplement ainsi : **IUR = Impact terrain / Coût numérique global**. Le numérateur regroupe les effets utiles observables sur le terrain, comme les déchets localisés ou retirés, les actions réalisées, les zones nettoyées, les participants mobilisés et les rapports effectivement transmis. Le dénominateur regroupe les coûts numériques et matériels estimés, notamment l’énergie, le CO2e, l’eau, le stockage, les transferts et les services tiers.
...
Le dépôt fournit une photographie du projet à une date donnée. Au 13 mai 2026, il contient **1 366 fichiers source** et **223 822 lignes source** hors dépendances, builds, documentation, fichiers publics et lockfiles. Cette photographie inclut principalement le code applicatif, les scripts, la configuration et les fichiers de travail utiles à l’audit.
...
Le projet a été créé le 20 février 2026 et la période active retenue s’étend jusqu’au 13 mai 2026. Sur cette base, l’hypothèse de travail retient environ **100 h** de développement assisté par IA, auxquelles s’ajoutent environ **20 h** de rédaction, restructuration et intégration documentaire au 16 mai 2026. Ces chiffres ne décrivent pas une productivité universelle : ils servent à cadrer la suite du raisonnement.
...
| ChatGPT 5.5 LLM | 20 % | 20 h | prompts, réflexions, sources, rédaction du rapport IA |
...
- limiter les builds, previews et relances provoqués par de simples changements documentaires.

## Limites méthodologiques et contrôles futurs

### Incertitudes principales

Les principales incertitudes concernent le nombre réel de requêtes IA, leur type, la localisation effective des calculs, la part de modèles légers ou lourds, le volume de builds déclenchés par les itérations, le stockage réel des photos en production, le trafic futur sur les cartes et rapports, ainsi que le nombre d’actions terrain réellement attribuables à CleanMyMap.
...
Codex propose toutefois des outils de suivi. OpenAI indique que l’usage peut être consulté dans le tableau de bord Codex, et que la commande `/status` permet de voir les limites restantes pendant une session Codex CLI. ([OpenAI Développeurs](https://developers.openai.com/codex/pricing)) Cette information est utile, mais elle reste un indicateur d’usage interne, pas une véritable mesure environnementale ou une comptabilité complète des ressources mobilisées.
...
Les règles opérationnelles de sobriété, de découpage des prompts et de validation humaine sont détaillées plus haut dans la Partie I, où elles sont rattachées à la stratégie globale de réduction.
...
Les tarifs varient fortement selon le modèle. À titre d’ordre de grandeur, OpenAI indique par exemple que GPT-5.4 mini est facturé **0,75$ par million de tokens en entrée** et **4,50$ par million de tokens en sortie** dans l’API, tandis que des modèles plus puissants coûtent davantage. [@openai_api_pricing] Cela signifie qu’une même session de travail peut coûter quelques dollars avec un modèle léger, mais beaucoup plus avec un modèle haut de gamme, surtout si elle mobilise un contexte long et produit beaucoup de sortie.
...
L'argent destinée au développement de **CleanMyMap** participe aussi à un système économique très concentré, dans lequel les bénéfices financiers et les coûts sociaux ne sont pas répartis de manière symétrique.
...
Anthropic a montré qu’un modèle entraîné à plaire à l’utilisateur peut parfois privilégier une réponse qui épouse ses croyances plutôt qu’une réponse plus vraie ou plus prudente [[1]0].
...
L'évaluation systémique d'un projet nécessite d'anticiper son cycle de vie logiciel, notamment les coûts induits par la forme de l'application, sa complexité, sa sécurité et sa dette produit.
...
## Annexe C — Foire aux questions (FAQ) {#annexe-c-faq}

Cette annexe regroupe l'ensemble des questions-réponses initialement dispersées dans les parties III et VII du rapport.

### Réponses courtes et rapides

- **Pourquoi utiliser l'IA si elle consomme de l'énergie ?** Parce que son usage reste un moyen et non une finalité. Elle a servi à accélérer le développement, améliorer la qualité, renforcer la documentation et les tests, dans des ordres de grandeur limités à l'échelle du projet et sous contrainte de sobriété. Elle n'est retenue que lorsqu'elle accélère un service utile plus qu'elle n'ajoute de coût durable.

- **Ce projet n'ajoute-t-il pas du numérique à un problème physique ?** Ce risque existe si l'outil ne change rien au terrain. CleanMyMap n'est défendable que s'il réduit les frictions réelles : signalements dispersés, photos non centralisées, doublons, pertes d'information, coordination lente et reporting difficile.

- **Comment éviter que le site devienne une usine à gaz ?** En gardant un noyau fonctionnel sobre : signalement, carte, cleanwalks, photos compressées, modération et export simple. Les couches plus lourdes ne doivent être ajoutées ou conservées que si les usages prouvent leur utilité.

- **Comment prouvez-vous que les bénéfices terrain existent vraiment ?** Par des indicateurs concrets : nombre de signalements validés, cleanwalks organisées, participants, déchets retirés, zones nettoyées, rapports transmis et actions qui n'auraient probablement pas eu lieu sans la plateforme. L'IUR, la sobriété technique et la comparaison avec des alternatives plus simples complètent cette évaluation.

- **Pourquoi ne pas utiliser seulement des outils existants comme Google Maps ou un tableur ?** Ces outils peuvent dépanner, mais ils laissent souvent les données dispersées entre cartes bricolées, fichiers, messages et photos. CleanMyMap vise à centraliser signalement, historique, preuve et coordination dans un même flux plus exploitable.

- **Comment limitez-vous les effets rebond liés aux déplacements ?** En faisant de la carte et de l'historique des outils d'orientation, pas de simple consultation. L'objectif est d'éviter les repérages inutiles, de mieux regrouper les actions et de concentrer les déplacements sur les interventions réellement utiles.

- **Comment protégez-vous les données des utilisateurs ?** En limitant les données collectées, en encadrant les accès, en anonymisant ce qui doit l'être et en refusant d'envoyer à l'IA des informations sensibles, des secrets techniques ou des données personnelles non nécessaires.

- **Que se passe-t-il si les services cloud deviennent trop chers ou indisponibles ?** C'est un risque identifié. Il est partiellement réduit par une architecture exportable, des formats simples, des fonctions désactivables et la possibilité de revenir à des usages plus sobres sans rendre le projet dépendant d'une seule brique critique évitable.

- **L'IA est-elle vraiment neutre ?** Non. Ses réponses dépendent de ses données d'entraînement, de son alignement, du contexte, de la langue et des garde-fous. Dans CleanMyMap, cela impose de limiter l'IA à des tâches d'assistance non sensibles, de relire ses sorties et de ne jamais lui déléguer une autorité décisionnelle.
...
Non, car il faut distinguer le *coût de conception* (ponctuel) du *gain d'usage* (récurrent). L’IA a consommé de l'énergie pour aider à coder et structurer CleanMyMap. Cependant, ce coût initial agit comme un levier : il a permis de sortir rapidement un outil qui coordonne des dizaines de "cleanwalks" physiques. L'énergie dépensée une fois par le modèle IA est largement amortie par les kilos de déchets (notamment plastiques) retirés durablement des écosystèmes avant leur dégradation en microplastiques.

#### N'est-il pas contradictoire d'utiliser l'IA générative pour créer un site censé être sobre ?

Le paradoxe n'est qu'apparent. L'IA a été utilisée comme outil d'optimisation : elle a aidé à générer des scripts de compression d'images (WebP/AVIF), à refactoriser le code pour le rendre moins lourd, et à éliminer des dépendances inutiles. En acceptant une dépense énergétique "amont" (requêtes IA pendant le développement), le projet garantit une économie énergétique "aval" (un site web léger qui consomme très peu de bande passante et de CPU pour chaque visiteur pendant des années).

#### Un projet écologique local ne devrait-il pas se passer de technologies de pointe ?

Sans l'IA, le développement d'une plateforme unifiée, sécurisée et performante aurait nécessité des moyens financiers et humains hors de portée pour une initiative naissante. L'IA permet à des citoyens ou de petites associations de se doter d'outils numériques de niveau professionnel pour mobiliser localement. Elle démocratise la capacité à agir à grande échelle sans nécessiter de levées de fonds ni de lourdes infrastructures initiales.
