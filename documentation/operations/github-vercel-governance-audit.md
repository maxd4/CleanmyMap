...
| Vercel - previews / Ignored Build Step | `apps/web/vercel.json` délègue désormais l'`ignoreCommand` à `apps/web/scripts/ignored-build-step.mjs`, qui compare les deux commits Vercel et ne saute que les changements explicitement classés hors build web | Réduire les builds inutiles sans masquer un changement web | Maintenir la liste des entrées build réelles et conserver le fallback fail-safe qui construit en cas de référence Git, diff ou chemin incertain | Moyenne |
