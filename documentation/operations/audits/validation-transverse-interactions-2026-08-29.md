...
| Authentification | `/sign-in`, `/sign-up` | anonyme | connexion, inscription, `returnTo` | précisément au moment de l’accès privé ou de la mutation | session Clerk établie puis retour vers la cible, onboarding si nécessaire | PAGE_OK | INTERACTION_OK | END_TO_END_OK | Clerk, `proxy.ts`, onboarding | `E2E-3C-AUTH-RETURN-DRAFT` : session Clerk Development E2E réelle établie via le harness officiel, retour exact vers `/sections/route` vérifié ; le cas d’inscription/onboarding reste hors de cette preuve. |
...
| Assistant itinéraire | `/sections/route` | anonyme puis connecté si nécessaire | contraintes publiques, puis demande de recommandation | aucune pour lire/remplir ; auth au clic de recommandation | formulaire public conservé puis cible Clerk atteinte au moment protégé | PAGE_OK | INTERACTION_OK | END_TO_END_OK | `/api/route/recommend`, Clerk, géocodage | `E2E-3C-AUTH-RETURN-DRAFT` : sept contraintes non par défaut restaurées après le retour Clerk et après reload ; bouton de calcul disponible mais non cliqué, aucun `POST /api/route/recommend`. Le calcul réel reste non prouvé/bloqué backend jusqu’à la campagne Supabase. |
...
## Campagne E2E n°3C — authentification Clerk tardive et reprise de l’itinéraire

Exécution Playwright locale ciblée le 30 août 2026 avec `CMM_DISABLE_DEV_AUTH_BYPASS=1`,
sans Docker ni Supabase local. Le harness officiel `@clerk/testing` a utilisé un
Testing Token, un utilisateur Clerk Development E2E et une session navigateur
réelle ; le teardown a supprimé l’utilisateur créé automatiquement. Aucun
calcul d’itinéraire, accès Supabase ou effet distant métier n’a été déclenché.

| statut | preuve | portée |
|---|---|---|
| `END_TO_END_OK` | `E2E-3C-AUTH-RETURN-DRAFT` : depuis une session anonyme, `/sections/route` est utilisable publiquement ; sept contraintes non par défaut sont saisies ; aucun POST same-origin ne part ; l’action explicite atteint `/sign-in?redirect_url=%2Fsections%2Froute` ; l’authentification Clerk officielle établit une session réelle et le retour exact vers `/sections/route` est atteint ; les sept contraintes sont visibles après retour et après reload ; le bouton `Calculer la recommandation` est disponible mais n’est pas cliqué. | authentification tardive et reprise du brouillon |
| `END_TO_END_OK` | `E2E-3C-NEGATIVE-CASES` : contexte anonyme sans bouton de calcul direct ; retour vers la cible sans session valide reste anonyme et sans action protégée ; brouillon corrompu et version de schéma incompatible restaurés sur les defaults canoniques sans exception ; aucune boucle d’auth/navigation ni mutation same-origin non sûre. | négatifs et defaults sûrs |
| `END_TO_END_OK` | Test unitaire `route-draft-storage.test.ts` : clé dédiée, schéma `1`, seuls les sept champs utiles, validation bornée par champ et stockage indisponible toléré ; un contexte navigateur neuf conserve les defaults et reste anonyme. | contrat de persistence locale bornée |
| `BLOCKED_BACKEND` | Le calcul réel de recommandation reste explicitement non prouvé : le bouton n’a pas été cliqué afin de ne déclencher ni Supabase ni progression métier. | `/api/route/recommend` et backend |

La preuve structurée est conservée dans `artifacts/playwright/route-campaign-3c/evidence.json`.
La cloche de notifications a produit une erreur de lecture auxiliaire sur le
retour authentifié, indépendante du brouillon et du calcul ; elle n’a généré
aucune mutation et n’est pas utilisée comme preuve de succès de la campagne.
...
| La saisie préparatoire du signalement ne pouvait pas être distinguée d’un blocage d’authentification de page. | Le formulaire complet appliquait un overlay/fieldset bloquant avant l’envoi, alors que la doctrine autorise la préparation publique. | Retrait du blocage transparent, ajout de CTA d’authentification au moment de l’envoi et sauvegarde locale des choix non sensibles dans `sessionStorage`, restaurable après authentification ; les anciens brouillons sont normalisés sans coordonnées puis supprimés après création réussie. | Les groupes et la position sont visibles dans Playwright ; aucune création puis relecture persistée n’a pu être menée à cause de la boucle Clerk. |
