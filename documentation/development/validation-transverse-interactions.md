...
| Carte des actions | `/actions/map` | anonyme/connecté | filtres date/zone/catégorie, couches, popup, géométrie, reset, rechargement | aucune pour lecture et export local | données publiques approuvées affichées, sélection synchronisée, carte lisible | PAGE_OK | NON PROUVÉ | NON PROUVÉ | `/api/actions/map`, `/api/actions/map/initial-nearest`, MapLibre, géoloc fallback | Route navigateur `200` et lecture API anonyme `200` avec `count=10` (`HTTP-A1`), mais filtres/couches/popup non validés par clic stable. |
