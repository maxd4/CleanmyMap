...
from src.maintenance._common import iter_text_files
...
def _rule_required_docs(root: Path) -> CleanupAuditRuleResult:
    required_files = [
        ...
        root / "documentation" / "operations" / "README.md",
        root / "documentation" / "pages_site" / "INDEX.md",
    ]
    ...
    if missing_files:
        return CleanupAuditRuleResult(
            ...
            recommendations_fr=[
                "Créer les pages documentation manquantes.",
    ...
    documentation_readme = (root / "documentation" / "README.md").read_text(encoding="utf-8")
    operations_readme = (root / "documentation" / "operations" / "README.md").read_text(encoding="utf-8")
    missing_refs: list[str] = []
    if "documentation/README.md" not in readme:
        missing_refs.append("README: reference à documentation/README.md")
    if "operations/README.md" not in documentation_readme:
        missing_refs.append("documentation README: reference à operations/README.md")
    if "pages_site/INDEX.md" not in documentation_readme:
        missing_refs.append("documentation README: reference à pages_site/INDEX.md")
    if "MAINTENANCE.md" not in operations_readme:
        missing_refs.append("operations README: référence à MAINTENANCE.md")
    if "CHANGELOG.md" not in operations_readme:
        missing_refs.append("operations README: référence à CHANGELOG.md")
    ...
    if missing_refs:
        return CleanupAuditRuleResult(
            ...
            title_en="Required documentation wiring",
            status="error",
            message_fr="Le chaînage documentaire est incomplet (README/Documentation).",
            message_en="Documentation wiring is incomplete (README/Documentation).",
            recommendations_fr=[
                "Mettre à jour README et documentation ensemble pour chaque changement important.",
                "Ajouter les références de maintenance manquantes.",
            ],
            recommendations_en=[
                "Update README and documentation together for each significant change.",
    ...
    return CleanupAuditRuleResult(
        ...
        title_en="Required documentation wiring",
        status="ok",
        message_fr="Le chaînage README/Documentation est cohérent.",
        message_en="README/Documentation wiring is consistent.",
    )
