...
def resolve_db_path(
    ...
    *,
    ...
    """Resolve runtime DB path with env override and OS fallback."""
    env_map = env if env is not None else os.environ
    env_path = env_map.get("CLEANMYMAP_DB_PATH") or env_map.get("SQLITE_DB_PATH")
    if env_path:
        return Path(env_path).expanduser()

    normalized_platform = (platform_name or os.name).lower()
    home_path = home if home is not None else Path.home()

    if normalized_platform in {"nt", "windows"}:
        local_app_data = env_map.get("LOCALAPPDATA")
        if local_app_data:
            return Path(local_app_data) / "CleanMyMap" / "runtime" / DB_FILENAME
        return home_path / "AppData" / "Local" / "CleanMyMap" / "runtime" / DB_FILENAME

    xdg_state_home = env_map.get("XDG_STATE_HOME")
    if xdg_state_home:
        return Path(xdg_state_home) / "cleanmymap" / "runtime" / DB_FILENAME
    return home_path / ".local" / "state" / "cleanmymap" / "runtime" / DB_FILENAME
...
def get_connection():
    """Cree une connexion SQLite vers la base de donnees."""
...
def init_db():
    ...
    c.execute("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, email TEXT UNIQUE, role TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
