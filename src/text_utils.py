        ...
        for source_encoding in ("cp1252", "latin-1"):
            for encode_errors, decode_errors in (("strict", "strict"), ("ignore", "ignore")):
                ...
                if candidate_score < best_score:
                    best, best_score, improved = candidate, candidate_score, True
...
def sanitize_dataframe_text(df: pd.DataFrame) -> pd.DataFrame:
    """Nettoie les colonnes texte d'un DataFrame de manière vectorisée ou via map."""
    if df.empty: return df
    cleaned = df.copy()
    cleaned.columns = [repair_mojibake_text(str(col)).strip() for col in cleaned.columns]
    text_cols = cleaned.select_dtypes(include=["object"]).columns
    for col in text_cols:
        cleaned[col] = cleaned[col].map(repair_mojibake_text)
    return cleaned

def _clean_streamlit_arg(value):
    if isinstance(value, str): return repair_mojibake_text(value)
    if isinstance(value, (list, tuple)):
        return type(value)([repair_mojibake_text(v) if isinstance(v, str) else v for v in value])
    return value
...
def patch_streamlit_text_api(st_module) -> None:
    ...
    for name in targets:
        ...
        if not callable(original) or getattr(original, "__cm_text_patched__", False): continue
        def wrapper(*args, __original=original, **kwargs):
            ...
            for key in ("label", "help", "placeholder"):
                if key in kwargs: kwargs[key] = _clean_streamlit_arg(kwargs[key])
