...
This project is indexed by GitNexus as **CleanmyMap** (19002 symbols, 40339 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
### Isolation obligatoire avant analyse GitNexus

Avant toute analyse GitNexus d’impact ou de dépendances :

- ne jamais analyser un workspace contenant plusieurs lots non commités ;
- utiliser un clone propre ou une branche isolée contenant uniquement le lot étudié ;
- considérer les résultats GitNexus invalides si l’arbre contient des modifications hors périmètre ;
- utiliser `rg` et l’analyse des imports comme fallback si l’isolation n’est pas possible ;
- signaler explicitement l’état dirty et ne pas présenter le rayon GitNexus comme une preuve fiable.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/CleanmyMap/context` | Codebase overview, check index freshness |
| `gitnexus://repo/CleanmyMap/clusters` | All functional areas |
| `gitnexus://repo/CleanmyMap/processes` | All execution flows |
| `gitnexus://repo/CleanmyMap/process/{name}` | Step-by-step execution trace |
