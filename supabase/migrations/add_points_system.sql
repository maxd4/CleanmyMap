...
CREATE TABLE IF NOT EXISTS public.points_ledger (
  ...
  reason text,
  source_event text, -- e.g. 'action_validated', 'spot_cleaned', 'quiz_completed'
  source_id text, -- reference to the source entity (actionId, spotId, etc)
  created_at timestamp with time zone default now(),
...
-- Trigger to automatically update user_points total when ledger entry is added
...
BEGIN
  ...
  -- Create user_points row if it doesn't exist
