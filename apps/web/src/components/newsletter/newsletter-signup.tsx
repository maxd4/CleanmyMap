...
export function NewsletterSignup() {
  ...
  if (status === "success") {
    return (
      <div className="p-8 rounded-3xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 text-center space-y-4 animate-in fade-in zoom-in duration-500">
        ...
        <h3 className="text-xl font-black text-emerald-900 dark:text-emerald-400">Inscription Réussie !</h3>
        <p className="text-sm text-emerald-700 dark:text-emerald-500/80 max-w-sm mx-auto leading-relaxed">
          {message}. <br />
  ...
  return (
    <div className="relative group p-8 md:p-12 rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden transition-all hover:shadow-2xl hover:border-emerald-500/30">
      {/* Background Decor */}
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
        <div className="flex-1 space-y-4 text-center md:text-left">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest rounded-full">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative">
              <input
                ...
                className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:ring-2 focus:ring-emerald-500 focus:outline-none dark:text-white transition-all"
              />
              <button
                type="submit"
            ...
            <label className="flex items-start gap-3 cursor-pointer group/consent">
              <div className="relative mt-1">
                ...
                />
                <div className="w-5 h-5 border-2 border-slate-300 dark:border-slate-700 rounded bg-white dark:bg-slate-900 peer-checked:bg-emerald-500 peer-checked:border-emerald-500 transition-all" />
                <CheckCircle2 size={14} className="absolute inset-0 m-auto text-white opacity-0 peer-checked:opacity-100 transition-all" />
              </div>
              <span className="text-[11px] leading-tight text-slate-400 dark:text-slate-500 group-hover/consent:text-slate-600 dark:group-hover/consent:text-slate-400 transition-colors">
            ...
            {status === "error" && (
              <p className="text-xs font-bold text-rose-500 animate-in fade-in slide-in-from-top-2">{message}</p>
            )}
          </form>
        </div>
      </div>
