  ...
  return (
    <div className={`${compact ? "space-y-1 rounded-xl p-2" : "space-y-2 rounded-2xl p-3"} border border-slate-200/70 bg-white/70 dark:border-slate-700/70 dark:bg-slate-900/60`}>
      <label className={compact ? "block space-y-1" : "block space-y-1.5"}>
        <span className={`block cmm-text-caption font-semibold ${isLight ? "text-slate-600" : "text-slate-500"}`}>
          {compact ? "Zone active" : "Territoire consulté"}
        </span>
        <select
          value={currentOptionValue}
          onChange={(event) => onChange(event.target.value)}
          className={`w-full rounded-xl border px-3 py-2 text-xs font-semibold outline-none focus:ring-2 focus:ring-pink-400 ${isLight ? "border-slate-200 bg-white text-slate-800" : "border-slate-700 bg-slate-950 text-slate-100"}`}
        >
