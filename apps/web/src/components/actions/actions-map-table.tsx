  ...
  if (items.length === 0) {
    return (
      <section className={`rounded-2xl border border-slate-200 bg-white shadow-sm ${compact ? "p-4" : "p-6"}`}>
        <p className={compact ? "text-xs text-slate-600" : "text-sm text-slate-600"}>Aucun point géolocalisé pour cette période.</p>
      </section>
    );
  }
  ...
  return (
    <section className={`rounded-2xl border border-slate-200 bg-white shadow-sm ${compact ? "p-4" : "p-6"}`}>
      <div className={`flex items-center justify-between ${compact ? "mb-3" : "mb-4"}`}>
        <h2 className={compact ? "text-lg font-semibold text-slate-900" : "text-xl font-semibold text-slate-900"}>Journal des actions (YTD)</h2>
      ...
      {!isVisible ? (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-5 py-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-900">Journal masqué par défaut</p>
              ...
              return (
                ...
                <td className="px-2 py-2">
                  {drawing ? (
                    ...
                    <span className="text-xs text-slate-400">Point discret</span>
                  )}
                </td>
