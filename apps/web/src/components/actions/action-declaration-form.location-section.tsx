          ...
          {routePreviewStatus === "processing"
            ...
            : routePreviewStatus === "ready"
              ...
              : routePreviewStatus === "error"
                ? "Aperçu partiel."
                : "Aperçu en attente."}
          {routePreviewMessage ? ` ${routePreviewMessage}` : null}
        </div>

        {!displayDrawing ? (
          <div className="mt-3 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-3 cmm-text-small cmm-text-muted">
            Saisis un lieu pour voir l&apos;aperçu dans le périmètre Paris + proche
            banlieue. En mode complet, active l&apos;ajustement manuel pour
            dessiner la zone.
          </div>
        ) : null}
      </div>
