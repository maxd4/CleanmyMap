...
const WASTE_TYPES = [
 ...
 { id:"mixte", label:"Mixte", icon:"🥡", color:"bg-slate-100 cmm-text-secondary border-slate-200" },
];
...
export function QuickSignalementForm() {
 ...
 <h2 className="text-2xl font-bold cmm-text-primary">Merci !</h2>
 <p className="cmm-text-muted">Votre signalement a été enregistré avec succès.</p>
 </div>
 ...
 <Link 
 href="/dashboard"
 className="w-full py-4 rounded-2xl bg-slate-100 cmm-text-secondary font-bold text-lg hover:bg-slate-200 transition"
 >
 RETOUR DASHBOARD
 </Link>
 </div>
 ...
 <h3 className="text-lg font-bold cmm-text-primary uppercase tracking-tight">1. Choisissez le type</h3>
 ...
 <span className="font-bold uppercase tracking-wider cmm-text-caption">{type.label}</span>
 </button>
 ))}
 </div>
 </div>
 ...
 <div className="space-y-4">
 <h3 className="text-lg font-bold cmm-text-primary uppercase tracking-tight">2. Photo (Optionnel)</h3>
 <label className="flex flex-col items-center justify-center gap-4 py-8 rounded-[2rem] border-4 border-dashed border-slate-200 bg-slate-50 cmm-text-muted cursor-pointer hover:border-emerald-300 hover:bg-emerald-50 transition-colors">
 <Camera size={32} />
 <span className="cmm-text-small font-bold">PRENDRE UNE PHOTO</span>
 ...
 className="w-full py-5 rounded-[2rem] bg-slate-900 text-white font-bold text-xl shadow-2xl flex items-center justify-center gap-3 disabled:opacity-30 disabled:grayscale transition-all hover:scale-[1.02] active:scale-95"
 >
 {isSubmitting ? (
 <>
...
"ENVOYER LE SIGNALEMENT"
 ...
 <p className="text-center mt-4 cmm-text-caption font-bold text-rose-500 uppercase tracking-widest animate-pulse">
