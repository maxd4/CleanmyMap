  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 bg-white p-6 rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
      ...
      <div className="lg:w-80 space-y-8 flex flex-col">
        <header>
          <div className="flex items-center gap-2 text-emerald-600 font-bold uppercase tracking-widest text-xs mb-2">
            <Zap size={14} /> AI ENGINE v1
          </div>
          <h3 className="text-2xl font-black text-slate-900 leading-tight">Itinéraire Intelligent.</h3>
        </header>

        <div className="space-y-4">
          <div className="flex justify-between items-end">
            <span className="text-xs font-bold text-slate-500 uppercase">Temps estimé</span>
            <span className="text-xl font-black text-slate-900">{timeMinutes} min</span>
          </div>
          <input 
            type="range" 
            min="15" 
        ...
        <div className="flex-1 space-y-3">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Points de collecte optimisés</p>
          {routeSpots.map((spot, idx) => (
            <div key={spot.id} className="flex gap-3 items-center p-3 rounded-xl bg-slate-50 border border-slate-100">
              ...
              <div className="flex-1">
                <p className="text-xs font-bold text-slate-900">Hotspot {spot.id}</p>
                <p className="text-[10px] text-slate-500">{spot.wasteKg}kg • {spot.butts} mégots</p>
              </div>
              <div className={`text-[10px] font-black px-2 py-1 rounded ${spot.score > 50 ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600'}`}>
                {spot.score}%
              </div>
            </div>
          ))}
      ...
      <div className="flex-1 h-[500px] rounded-2xl overflow-hidden border border-slate-200 relative">
        ...
        <div className="absolute top-4 right-4 bg-white/90 backdrop-blur px-3 py-1.5 rounded-lg border border-slate-200 text-[10px] font-bold text-slate-500 shadow-sm sm:block hidden">
