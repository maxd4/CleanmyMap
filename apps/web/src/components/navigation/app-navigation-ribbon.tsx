...
function getSpaceToneClasses(spaceId: keyof typeof SPACE_ICON_MAP): {
  ...
  const toneBySpace = {
    home: {
      active:
        "border-slate-300 bg-white text-slate-900 shadow-[0_8px_20px_-12px_rgba(15,23,42,0.45)] dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100",
      inactive:
        "border-slate-200/80 bg-slate-50 text-slate-700 hover:border-slate-300 hover:bg-white hover:text-slate-900 dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-200 dark:hover:border-slate-500 dark:hover:bg-slate-900 dark:hover:text-slate-50",
    },
    act: {
      active:
          ...
          <div className="flex flex-col gap-3 py-2.5 sm:py-3">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:gap-4">
              <div className="hidden min-w-[11rem] items-center gap-2 xl:flex">
                <Link
                  href="/"
                  className="inline-flex items-center rounded-full border border-slate-200/90 bg-white px-3.5 py-2 text-[11px] font-bold uppercase tracking-[0.18em] text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-200 hover:bg-emerald-50/70 hover:text-emerald-800 hover:shadow-md dark:border-slate-800 dark:bg-slate-900/90 dark:text-slate-200 dark:hover:border-emerald-700 dark:hover:bg-slate-800 dark:hover:text-emerald-200"
                  ...
                  {spaces.map((space) => {
                    ...
                    return (
                      <Link
                        ...
                        className={[
                          "inline-flex min-h-11 shrink-0 items-center gap-2 rounded-full border px-3.5 py-2 text-[11px] font-semibold transition sm:text-xs",
              ...
              <div className="flex flex-wrap items-center justify-end gap-2 lg:flex-nowrap">
                <Link
                  href="/explorer"
                  className="inline-flex shrink-0 items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-[11px] font-semibold text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-800 hover:shadow-md dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-emerald-700 dark:hover:bg-slate-800 dark:hover:text-slate-100 sm:text-xs"
                >
                ...
                >
                  <summary className="inline-flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-[11px] font-semibold text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-800 hover:shadow-md dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-emerald-700 dark:hover:bg-slate-800 dark:hover:text-slate-100 sm:px-3.5 sm:text-xs [&::-webkit-details-marker]:hidden">
                ...
                <Show when="signed-out">
                  <div className="flex items-center gap-2">
                    <SignInButton mode="modal">
                      <button className="rounded-full border border-slate-200 bg-white px-3 py-2 text-[11px] font-semibold text-slate-700 transition hover:border-emerald-200 hover:text-emerald-800 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-emerald-700 dark:hover:text-emerald-200 sm:text-xs">
                        {locale === "fr" ? "Connexion" : "Sign in"}
                      </button>
                    </SignInButton>
                    <SignUpButton mode="modal">
                      <button className="rounded-full bg-emerald-600 px-4 py-2 text-[11px] font-semibold text-white shadow-sm shadow-emerald-600/15 transition hover:bg-emerald-700 sm:text-xs">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex min-w-0 items-center gap-2 overflow-x-auto scrollbar-none">
                  {activeSpaceItems.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className={[
                          "inline-flex min-h-9 shrink-0 items-center rounded-full border px-3 py-1.5 text-[11px] font-medium transition sm:text-xs",
                          isItemActive
                            ? "border-emerald-200 bg-emerald-50 text-emerald-900 shadow-[0_8px_18px_-12px_rgba(16,185,129,0.45)] dark:border-emerald-700 dark:bg-emerald-950/70 dark:text-emerald-100"
                            : "border-slate-200 bg-white text-slate-600 hover:border-emerald-200 hover:bg-emerald-50 hover:text-slate-800 hover:shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-emerald-700 dark:hover:bg-slate-800 dark:hover:text-slate-100",
                        ].join(" ")}
                      >
                        {item.label[locale]}
                        {isItemActive ? (
                  ...
                  {activeSpaceItems.length === 0 ? (
                    <span className="inline-flex min-h-9 shrink-0 items-center rounded-full border border-dashed border-slate-300 px-3 py-1.5 text-[11px] text-slate-500 dark:border-slate-700 dark:text-slate-400">
                      {locale === "fr"
