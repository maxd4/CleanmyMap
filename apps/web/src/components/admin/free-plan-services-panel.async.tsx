...
export function FreePlanRefreshButton({
  isRefreshing,
  onRefresh,
}: {
...
export function FreePlanLoadingState() {
  return (
    <div
      ...
      aria-live="polite"
      aria-busy="true"
      aria-label="Chargement de la fiche des plans gratuits"
    >
...
export function FreePlanErrorState({
  isRefreshing,
  onRetry,
}: {
  isRefreshing: boolean;
  ...
  return (
    <CmmFeedback
      tone="error"
      title="Fiche des plans gratuits indisponible"
      action={
        <CmmButton
          type="button"
          tone="primary"
    ...
    >
      Impossible de charger la fiche des plans gratuits. Vérifiez la lecture
      de l&apos;estimateur d&apos;impact et l&apos;API services.
    </CmmFeedback>
  );
}
...
export function FreePlanServicesDataView({
  ...
  panelModel: ReturnType<typeof buildFreePlanServicesPanelModel>;
  serviceHealth: ServicesPayload["services"];
}) {
  const {
    quotaServices,
    previousServices,
