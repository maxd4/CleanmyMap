  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h3 className="text-lg font-bold text-slate-900">{title || "Répartition territoriale"}</h3>
          <p className="text-sm text-slate-500">Par arrondissement (Paris) ou ville</p>
        </div>
        
        <select 
          value={metric} 
          onChange={(e) => setMetric(e.target.value as any)}
          className="bg-slate-50 border border-slate-300 text-slate-700 text-sm rounded-lg p-2 outline-none focus:ring-2 focus:ring-emerald-500"
        >
      ...
      <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t border-slate-100">
        {chartData.slice(0, 4).map((item, i) => (
          <div key={item.area}>
            <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold">{item.area}</p>
