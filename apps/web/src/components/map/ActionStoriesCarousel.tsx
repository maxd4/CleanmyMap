...
export function ActionStoriesCarousel({ items, onOpenAction, compact = false }: ActionStoriesCarouselProps) {
  const [index, setIndex] = useState(0);

  if (!items.length) return null;

  return (
    <div className={compact ? "relative w-full py-4" : "relative w-full py-8"}>
      <div className={compact ? "mb-4 flex items-center justify-between px-2" : "mb-6 flex items-center justify-between px-4"}>
        <div>
          <h3 className={compact ? "text-base font-semibold tracking-[-0.02em] text-slate-950" : "text-lg font-semibold tracking-[-0.02em] text-slate-950"}>
            Dernières actions
          </h3>
      ...
      <div className={compact ? "relative h-[320px] w-full perspective-1000" : "relative h-[400px] w-full perspective-1000"}>
        <AnimatePresence mode="popLayout">
          {items.slice(index % items.length, (index % items.length) + (compact ? 2 : 3)).map((item, i) => {
            ...
            return (
              ...
              >
                <CmmCard className={compact ? "pointer-events-auto h-full w-full max-w-sm overflow-hidden border border-cyan-200/80 bg-cyan-50/95 shadow-[0_24px_56px_-32px_rgba(8,145,178,0.22)] backdrop-blur-3xl group" : "pointer-events-auto h-full w-full max-w-sm overflow-hidden border border-cyan-200/80 bg-cyan-50/95 shadow-[0_24px_56px_-32px_rgba(8,145,178,0.22)] backdrop-blur-3xl group"}>
                  ...
                  <div className={compact ? "relative z-20 flex h-full flex-col justify-end p-5" : "relative z-20 flex h-full flex-col justify-end p-6"}>
                    ...
                    <h4 className={compact ? "mb-2 text-lg font-semibold text-white line-clamp-2" : "mb-2 text-xl font-semibold text-white line-clamp-2"}>
                    ...
                    <CmmButton
                      variant="default"
                      tone="secondary"
                      className={compact ? "w-full border border-white/20 bg-white text-slate-950 py-2.5 shadow-lg shadow-slate-950/10 transition-colors hover:bg-cyan-100 active:bg-cyan-100 focus-visible:bg-cyan-100" : "w-full border border-white/20 bg-white text-slate-950 py-3 shadow-lg shadow-slate-950/10 transition-colors hover:bg-cyan-100 active:bg-cyan-100 focus-visible:bg-cyan-100"}
