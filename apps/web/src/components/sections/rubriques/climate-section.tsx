        ...
        <>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              {summaryKpis.map((kpi) => (
                ...
                >
                  ...
                  <p className={`mt-1 text-xs font-semibold ${kpi.delta.tone}`}>
                    {kpi.delta.text} vs periode precedente
                  </p>
                </div>
              ))}
            </div>
              ...
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
                ...
                <p className="mt-1">
                  ...
                  <span className="font-semibold">
                    {weakestIndicator.confidence}
                  </span>{" "}
                  sur{" "}
                  <span className="font-semibold">
                    {weakestIndicator.label}
                  </span>
                  .
                </p>
              </div>
              <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900">
                ...
                <p className="mt-1 font-semibold">
                  {firstDecision?.decision ??
                    "Consolider la qualite des donnees terrain."}
                </p>
              </div>
            </div>
            <p className="mt-3 text-xs text-slate-500">
              Proxies model {context.modelVersion} - facteurs date{" "}
              {context.factors.factorVersionDate} - recalcul{" "}
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {context.indicators.map((indicator) => {
              const previousValue =
                indicator.id === "volume"
                  ? context.comparison.previous.volumeKg
                  : indicator.id === "butts"
                    ? context.comparison.previous.butts
                    : indicator.id === "hours"
                      ? context.comparison.previous.citizenHours
                      : indicator.id === "co2_proxy"
                        ? context.comparison.previous.co2ProxyKg
                        : indicator.id === "plastic_leakage_proxy"
                          ? context.comparison.previous.plasticLeakageProxyKg
                          : context.comparison.previous.geocoverageRate;
              const delta = formatDeltaLine(
                indicator.value,
                previousValue,
                indicator.unit,
              );
              return (
                ...
                >
                  ...
                  <p className="mt-1 text-2xl font-semibold text-slate-900">
                    {indicator.value.toFixed(indicator.unit === "u" ? 0 : 1)}{" "}
                    {indicator.unit}
                  </p>
                  <p className={`mt-1 text-xs font-semibold ${delta.tone}`}>
                    {delta.text} vs periode precedente
                  </p>
                </article>
              );
            })}
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <article className="rounded-xl border border-slate-200 bg-white p-4">
              <h3 className="text-sm font-semibold text-slate-900">Methode</h3>
              <ul className="mt-2 space-y-2 text-sm text-slate-700">
                {context.methods.map((method) => (
                  ...
                  >
                    ...
                    <p>Source: {method.source}</p>
                    <p>
                      Frequence: {method.frequency} - version {method.version}
                    </p>
                  </li>
                ))}
              </ul>
            ...
            <article className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              <h4 className="mt-4 text-sm font-semibold text-slate-900">
                Ce qu&apos;il faut decider cette semaine
              </h4>
              <ul className="mt-2 space-y-2 text-sm text-slate-700">
