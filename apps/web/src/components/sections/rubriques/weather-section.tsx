    ...
    <div className="space-y-6">
      <div className="flex items-center gap-2 p-1 bg-slate-100 rounded-2xl w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition ${activeTab === "weather" ? "bg-white text-emerald-700 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}
        >
          <CloudSun size={18} />
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition ${activeTab === "kit" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}
        >
          <ClipboardCheck size={18} />
          {fr ? "Kit terrain" : "Field kit"}
          <span className="ml-1 px-1.5 py-0.5 rounded-full bg-slate-200 text-[10px] text-slate-600">{kitProgress}%</span>
        </button>
      </div>
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3">
                <button onClick={() => setZoneMode("auto")} className={`rounded-lg border px-3 py-1.5 text-xs font-semibold transition ${zoneMode === "auto" ? "border-slate-800 bg-slate-800 text-white" : "border-slate-300 bg-slate-50 text-slate-700 hover:bg-slate-100"}`}>
            ...
            {!isLoading && !error ? (
              <article className={`rounded-xl border shadow-sm p-4 ${riskTone}`}>
                <h3 className="text-sm font-semibold uppercase tracking-wide">{fr ? "Lecture décisionnelle" : "Decision reading"}</h3>
            ...
            {!isLoading && !error ? (
              <>
                <div className="bg-slate-100 p-1.5 flex gap-1 rounded-lg w-fit">
                  <button onClick={() => setActivePeriod("now")} className={`rounded-md px-4 py-1.5 text-xs font-semibold transition ${activePeriod === "now" ? "bg-white text-slate-900 shadow-sm" : "text-slate-600 hover:text-slate-900"}`}>0-6h (Nowcasting)</button>
                  <button onClick={() => setActivePeriod("j13")} className={`rounded-md px-4 py-1.5 text-xs font-semibold transition ${activePeriod === "j13" ? "bg-white text-slate-900 shadow-sm" : "text-slate-600 hover:text-slate-900"}`}>J+1 à J+3</button>
                  <button onClick={() => setActivePeriod("j7")} className={`rounded-md px-4 py-1.5 text-xs font-semibold transition ${activePeriod === "j7" ? "bg-white text-slate-900 shadow-sm" : "text-slate-600 hover:text-slate-900"}`}>Horizon J+7</button>
                </div>
                    ...
                    {nowcasting.map((point) => {
                      ...
                      return (
                        <article key={point.time} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm hover:border-emerald-200 transition">
                          <p className="font-semibold text-slate-900 text-xs border-b border-slate-100 pb-2 mb-2">{formatDateTimeShort(point.time)}</p>
                    ...
                    {j13.map((day) => {
                      ...
                      return (
                        <article key={day.day} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm hover:border-emerald-200 transition">
                          <p className="font-semibold text-slate-900 text-xs border-b border-slate-100 pb-2 mb-2">{day.day}</p>
                          <ul className="text-[11px] font-medium text-slate-600 space-y-1">
                ...
                {activePeriod === "j7" && (
                  <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                    <p className="text-sm font-bold text-slate-900">{j7.day}</p>
                    <ul className="mt-2 text-xs font-medium text-slate-600 space-y-1">
                ...
                <div className="grid gap-4 md:grid-cols-2 pt-2 border-t border-slate-100">
                  <article className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4 shadow-sm text-sm">
                    <h3 className="font-semibold text-emerald-900 mb-2">{fr ? "Fenêtres d'intervention" : "Recommended windows"}</h3>
                    <ul className="space-y-1.5">
                      {windows.recommended.slice(0, 3).map((w) => (
                        <li key={w.from} className="text-xs font-medium text-emerald-800">• {formatDateTimeShort(w.from)} : {w.reason}</li>
                      ))}
                    </ul>
                  </article>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] gap-6 items-start animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="space-y-4">
            <article className="rounded-3xl border border-slate-200 bg-white/90 p-5 shadow-md">
              <label className="flex flex-col gap-2 text-sm font-bold text-slate-900 mb-4">
              ...
              <div className="pt-4 border-t border-slate-100">
                <h2 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-3">{fr ? "Matériel" : "Equipment"}</h2>
          ...
          <div className="space-y-4">
            <article className="rounded-3xl border border-slate-200 bg-slate-50/50 p-6 shadow-inner">
              <h2 className="text-lg font-black text-slate-900 mb-4">{fr ? "Checklist Sécurité" : "Safety Checklist"}</h2>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <Link href="/actions/new" className="px-6 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 transition">
                  {fr ? "Démarrer l'action" : "Start action"}
                </Link>
                <Link href="/sections/guide" className="px-6 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl text-sm font-bold hover:bg-slate-50 transition">
