...
import { ArrowRight, HelpCircle, MapPin, Search } from "lucide-react";
...
function buildAnswer(question: string, locale: Locale): Answer {
...
function toneClasses(tone: Tone): { shell: string; badge: string; title: string } {
  switch (tone) {
    case "emerald":
      return {
...
export function RecyclingQuestionAssistant() {
  ...
  const copy = fr
    ? {
        ...
        subtitle:
          "Pose une question en langage simple: dans quelle poubelle, déchèterie ou point de collecte spécifique ?",
        placeholder: "Ex. : dans quelle poubelle je mets une pile usagée ?",
        helper:
          "Tu peux aussi demander si un objet se signale sur DansMaRue ou s'il faut plutôt une filière dédiée.",
    ...
    : {
        ...
        subtitle:
          "Ask in plain language: which bin, recycling center or dedicated collection point?",
        placeholder: "Ex. : which bin for a used battery?",
        helper:
          "You can also ask whether an item should be reported on DansMaRue or sent to a dedicated stream.",
  ...
  return (
    <section className={`rounded-3xl border p-6 shadow-sm ${classes.shell}`}>
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-2xl">
          ...
          <h3 className={`mt-2 text-2xl font-black tracking-tight ${classes.title}`}>
            {copy.subtitle}
          </h3>
          <p className="mt-2 text-sm cmm-text-secondary">{copy.helper}</p>
        </div>

        <div className="rounded-2xl border border-white/60 bg-white/70 px-4 py-3 text-sm font-semibold cmm-text-secondary shadow-sm">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <textarea
            ...
            className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm cmm-text-primary outline-none transition placeholder:text-slate-400 focus:border-emerald-400 focus:bg-white"
          />
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <button
              type="button"
              onClick={() => setQuestion("")}
              className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 transition hover:bg-slate-50"
            >
              <Search size={16} />
              {copy.clear}
            </button>
            <p className="text-sm cmm-text-secondary">
              {fr
                ? "Tu peux taper un objet, un lieu ou un cas d'usage."
                : "You can type an item, a place or a use case."}
            </p>
          </div>
        </div>

        <div
          className={`rounded-2xl border p-4 shadow-sm ${classes.shell} bg-white`}
          aria-live="polite"
        >
          ...
          <div
            className={`mt-3 inline-flex items-center rounded-full px-3 py-1 text-xs font-black uppercase tracking-[0.14em] ${classes.badge}`}
          >
            {answer.badge}
          </div>
          <h4 className={`mt-3 text-xl font-black tracking-tight ${classes.title}`}>{answer.title}</h4>
          <p className="mt-2 text-sm cmm-text-secondary">{answer.summary}</p>

          <ul className="mt-4 space-y-2">
            {answer.bullets.map((bullet) => (
              <li key={bullet} className="flex gap-2 text-sm cmm-text-secondary">
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-3">
            ...
            <p className="mt-1 text-sm font-semibold cmm-text-primary">{answer.nextStep}</p>
          </div>
          ...
          {answer.note ? (
            <div className="mt-3 rounded-2xl border border-slate-200 bg-white p-3">
              ...
              <p className="mt-1 text-sm cmm-text-secondary">{answer.note}</p>
            </div>
          ) : null}

          <p className="mt-4 text-xs cmm-text-secondary">
            {fr
              ? "Les règles locales peuvent varier. En cas de doute, le principe de base reste: propre et vide pour le recyclage, filière dédiée pour les piles, batteries et déchets spéciaux, et poubelle grise si le tri conseillé est impossible."
              : "Local rules can vary. When in doubt, the base rule is: clean and empty for recycling, dedicated streams for batteries and special waste, and the residual bin if the recommended sorting is not possible."}
          </p>

          <Link
            href="/learn/ressources"
            className="mt-4 inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-slate-900 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-slate-800"
          >
            {copy.cta}
            <ArrowRight size={16} />
          </Link>
