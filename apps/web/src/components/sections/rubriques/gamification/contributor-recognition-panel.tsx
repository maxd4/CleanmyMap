    ...
    <section className="space-y-6">
      <div className="flex items-center gap-3 px-2">
        ...
        <h3 className="text-[10px] font-black text-white uppercase tracking-[0.3em]">
          {fr ? "Reconnaissance sociale" : "Social recognition"}
        </h3>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-[3rem] border border-white/5 bg-slate-950/40 backdrop-blur-3xl p-8 text-sm text-slate-400">
          {fr
            ? "Aucune reconnaissance sociale vérifiée n'est encore disponible pour cette vue."
            : "No verified social recognition is available yet for this view."}
        </div>
      ) : (
        <div className="space-y-4">
          <div className="rounded-[2.25rem] border border-red-500/15 bg-red-500/[0.03] p-5">
            <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.25em] text-red-300">
              <Sparkles size={12} />
              {fr ? "Message de reconnaissance mis en avant" : "Featured recognition message"}
            </div>
            <p className="mt-3 text-sm font-semibold leading-relaxed text-white">
              {cards[0]?.thanksMessage}
            </p>
          </div>
