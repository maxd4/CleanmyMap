...
describe("legal content report form anti-spam contract", () => {
  it("sends the form start time instead of the click time", () => {
    expect(form).toContain('import { useEffect, useRef, useState } from "react";');
    expect(form).toMatch(/useEffect\(\(\) => \{\s*formStartedAt\.current = Date\.now\(\);\s*\}, \[\]\);/);
    expect(form).toContain("submittedAt: formStartedAt.current ?? Date.now(),");
  ...
  it("starts a fresh anti-spam window when another notification is displayed", () => {
    expect(form).toMatch(/setState\("idle"\);\s*formStartedAt\.current = Date\.now\(\);/);
  });
});
