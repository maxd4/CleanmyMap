...
export function PublishedAnnuaireReviewPanel({
  ...
  const reviewEntry = (
    ...
    if (confirmPhrase.trim().toUpperCase() !== REVIEW_CONFIRM_PHRASE) {
      setError(
        fr
          ? `Renseigne exactement "${REVIEW_CONFIRM_PHRASE}" pour valider la revue.`
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <label className="space-y-1 text-xs font-semibold text-slate-700">
          ...
          <input
            ...
            className="block min-w-[220px] rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-800 focus:border-emerald-500 focus:outline-none"
          />
        </label>
      </div>
        ...
        {items.map((entry) => (
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <span
                className={`rounded-full border px-2 py-1 text-[11px] font-semibold ${statusTone(entry.publicationStatus)}`}
              >
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button
                ...
                className="rounded-lg border border-rose-200 bg-white px-3 py-2 text-xs font-semibold text-rose-700 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {fr ? "Rejeter" : "Reject"}
              </button>
            </div>
