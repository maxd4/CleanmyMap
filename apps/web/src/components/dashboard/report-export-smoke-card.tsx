...
export function ReportExportSmokeCard() {
  ...
  async function runProbe() {
    ...
    try {
      ...
      setMessage(snippet.length > 0 ? "Endpoint CSV opérationnel." : "Endpoint CSV vide mais disponible.");
    } catch (error) {
      setProbeState("error");
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-2 text-sm text-slate-600">Accès direct au module exports + test rapide de disponibilité CSV.</p>

      <div className="mt-3 flex flex-wrap items-center gap-3">
        <Link href="/reports" className="inline-flex rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white hover:bg-emerald-700">
          Ouvrir le reporting
        </Link>
        <button
        ...
        >
          {probeState === "pending" ? "Test en cours..." : "Tester l'export CSV"}
        </button>
      </div>

      {probeState === "success" && message ? (
        <p className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700">{message}</p>
      ) : null}
      {probeState === "error" && message ? (
        <p className="mt-3 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700">{message}</p>
      ) : null}
    </article>
  );
}
