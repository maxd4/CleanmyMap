...
function QuestionStatusBadge({
  ...
  if (kind === "source" && label && href) {
    return (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="inline-flex items-center gap-2 rounded-full border border-sky-200 bg-sky-50 px-3 py-1 cmm-text-small font-black uppercase tracking-[0.18em] text-sky-800 transition hover:bg-sky-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-300/70"
      >
        {label}
  ...
  return (
    <span className="inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-3 py-1 cmm-text-caption font-black uppercase tracking-[0.18em] text-amber-800">
      À vérifier
    ...
    <section className="space-y-4 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="max-w-3xl">
          <p className="cmm-text-caption font-black uppercase tracking-[0.18em] text-slate-500">
        ...
        <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
          {BANK_SUMMARY.map((item) => (
            <div key={item.label.fr} className="rounded-[1.3rem] border border-amber-200 bg-amber-50 px-4 py-3 text-center shadow-sm">
              ...
              <p className="mt-1 cmm-text-caption font-black uppercase tracking-[0.18em] text-slate-500">{item.label[locale]}</p>
            </div>
          ))}
        </div>
      </div>
            ...
            <article key={track.id} className="rounded-[1.6rem] border border-amber-100 bg-amber-50/60 p-4 shadow-sm">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <p className="cmm-text-caption font-black uppercase tracking-[0.18em] text-amber-700">{getQuizSchoolTrackLabel(track.id, locale)}</p>
                  <h4 className="mt-1 text-base font-black tracking-tight text-slate-900">{getQuizSchoolTrackLabel(track.id, locale)}</h4>
                  <p className="cmm-text-body mt-1">{track.description[locale]}</p>
                </div>
              </div>
              <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-white bg-white px-3 py-2">
                <span className="cmm-text-caption font-black uppercase tracking-[0.18em] text-slate-500">{isFrench ? "Temps collectif" : "Collective step"}</span>
      ...
      <details className="group overflow-hidden rounded-[1.8rem] border border-amber-200 bg-white shadow-sm">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 outline-none transition hover:bg-amber-50/60 focus-visible:bg-amber-50/70 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-amber-300/70 md:px-6">
          <div className="min-w-0">
            <p className="cmm-text-caption font-black uppercase tracking-[0.18em] text-amber-700">{isFrench ? "Détails de la banque" : "Bank details"}</p>
          ...
          <span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1.5 cmm-text-caption font-black uppercase tracking-[0.18em] text-amber-800">Support</span>
        </summary>
                  ...
                  <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 outline-none transition hover:bg-amber-50/60 focus-visible:bg-amber-50/70 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-amber-300/70 md:px-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1.5 cmm-text-caption font-black uppercase tracking-[0.18em] text-amber-800">{questions.length} questions</span>
                  </summary>
                  <div className="border-t border-amber-100 p-5 md:p-6">
                      ...
                      {questions.map((question, index) => (
                        <article key={question.id} className="rounded-[1.6rem] border border-slate-200 bg-slate-50 p-4 shadow-sm">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <p className="cmm-text-caption font-black uppercase tracking-[0.18em] text-slate-500">{String(index + 1).padStart(2, "0")} · {question.typeLabel}</p>
                          ...
                          <div className="mt-4 space-y-3">
                            <div className="rounded-2xl border border-white bg-white p-3">
                              <p className="cmm-text-caption font-black uppercase tracking-[0.18em] text-amber-700">{isFrench ? "Réponse attendue" : "Expected answer"}</p>
                              <p className="mt-1 text-sm font-bold text-slate-900 md:text-base">{question.answer}</p>
                            </div>
                            <div className="rounded-2xl border border-white bg-white p-3">
                              <p className="cmm-text-caption font-black uppercase tracking-[0.18em] text-slate-500">{isFrench ? "Explication courte" : "Short explanation"}</p>
                              <p className="cmm-text-body mt-1">{question.explanation}</p>
                            </div>
                            <div className="rounded-2xl border border-amber-100 bg-amber-50 p-3">
                              <p className="cmm-text-caption font-black uppercase tracking-[0.18em] text-amber-700">À retenir</p>
