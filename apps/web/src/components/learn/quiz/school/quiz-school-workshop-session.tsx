    ...
    <div className="space-y-6" data-testid="quiz-school-workshop-session">
      ...
      {question ? (
        <CmmCard tone="amber" variant="elevated" className="p-5 md:p-8" as="section">
          ...
          <p className="cmm-text-small cmm-text-secondary mt-4">{getQuizUiCopy(locale, "session.school.workshop.vote")}</p>
        ...
        <CmmCard tone="amber" variant="elevated" className="p-5 md:p-8" as="section">
          ...
          {activity ? (
            <>
              ...
              <p className="cmm-text-body mt-4 font-medium"><span className="font-black text-slate-950">{locale === "fr" ? "Adaptation" : "Adaptation"} :</span> {activity.adaptation[locale]}</p>
              {activity.dataPoints ? (
      ...
      {state.phase === "bilan" ? (
        <CmmCard tone="amber" variant="elevated" className="p-5 md:p-8" as="section">
          ...
          <p className="cmm-text-body mt-3 max-w-3xl">{locale === "fr" ? "Ces résultats sont collectifs et restent dans ce navigateur : ils ne constituent ni un classement ni un profil d’élève." : "These results are collective and stay in this browser: they are neither a ranking nor a student profile."}</p>
          ...
          <p className="cmm-text-small cmm-text-secondary mt-3">{locale === "fr" ? "Les deux situations de transfert ne modifient pas la progression conceptuelle." : "The two transfer situations do not change concept progress."}</p>
          <div className="mt-8 grid gap-6 lg:grid-cols-2">
            <section aria-labelledby="workshop-takeaways">
              ...
              {summary.retainedNotions.length > 0 ? <ul className="cmm-text-small cmm-text-secondary mt-3 space-y-2 font-medium">{summary.retainedNotions.map((notion) => <li key={notion.fr} className="flex gap-2"><CheckCircle className="h-5 w-5 shrink-0 text-emerald-600" aria-hidden="true" /><span>{notion[locale]}</span></li>)}</ul> : <p className="cmm-text-small cmm-text-secondary mt-3">{locale === "fr" ? "Aucune notion retenue n’est démontrée par les réponses collectives." : "No retained notion is demonstrated by the collective answers."}</p>}
            </section>
            <section aria-labelledby="workshop-fragile">
              ...
              {summary.fragileNotions.length > 0 ? <ul className="cmm-text-small cmm-text-secondary mt-3 space-y-2 font-medium">{summary.fragileNotions.map((notion) => <li key={notion.fr} className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2">{notion[locale]}</li>)}</ul> : <p className="cmm-text-small cmm-text-secondary mt-3">{locale === "fr" ? "Aucune notion signalée dans le post-quiz." : "No idea flagged in the post-quiz."}</p>}
            </section>
          </div>
          ...
          <section aria-labelledby="workshop-territory" className="mt-8">
            ...
            {summary.territorialResources.length > 0 ? <ul className="mt-3 grid gap-3 md:grid-cols-3">{summary.territorialResources.map((resource) => <li key={resource.id} className="rounded-2xl border border-slate-200 bg-white p-4"><a className={`${FOCUS_RING} font-black text-amber-900 underline`} href={resource.officialUrl} target="_blank" rel="noreferrer">{resource.name[locale]}</a><p className="mt-1 text-xs font-black uppercase tracking-[0.12em] text-slate-500">{resource.territory[locale]}</p><p className="cmm-text-body mt-2">{resource.description[locale]}</p><p className="cmm-text-caption cmm-text-muted mt-2">{resource.audience[locale]}</p></li>)}</ul> : <p className="cmm-text-small cmm-text-secondary mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">{locale === "fr" ? "Aucune ressource locale validée n’est disponible pour le moment. Le professeur peut conserver les trois actions ci-dessus et rechercher une ressource officielle adaptée." : "No validated local resource is available at the moment. The teacher can keep the three actions above and look for a suitable official resource."}</p>}
          </section>
