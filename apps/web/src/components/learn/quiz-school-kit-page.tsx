  ...
  if (kind === "source" && label && href) {
    return (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="inline-flex items-center gap-2 rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-sky-800 transition hover:bg-sky-100"
      >
        {label}
  ...
  return (
    <LearnRubricShell
      ...
      cta={{
        ...
        label: { fr: "Ouvrir le quiz", en: "Open the quiz" },
      }}
    >
      <div className="space-y-8">
        ...
        <section className="space-y-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div className="max-w-3xl">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-amber-700">
                {isFrench ? "Banque test" : "Test bank"}
              </p>
              <h3 className="mt-1 text-2xl font-black tracking-tight text-slate-900">
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-600">
                {isFrench
                  ? "Cette première banque sert de base de travail, avec des questions piégeuses mais justes, une explication courte et une phrase à retenir."
                  : "This starter bank is a working base, with tricky but fair questions, a short explanation and a takeaway line."}
              </p>
            </div>
            <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
              {BANK_SUMMARY.map((item) => (
                <div key={item.label.fr} className="rounded-[1.3rem] border border-amber-200 bg-white px-4 py-3 text-center shadow-sm">
          ...
          <div className="space-y-4">
            {QUIZ_SCHOOL_TRACKS.map((track) => {
              ...
              return (
                <details
                  key={track.id}
                  open={track.id === "debat-classe"}
                  className="group overflow-hidden rounded-[2rem] border border-amber-200 bg-white shadow-sm"
                >
                  <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 outline-none transition hover:bg-amber-50/60 md:px-6">
                        ...
                        <article key={question.id} className="rounded-[1.6rem] border border-slate-200 bg-slate-50 p-4 shadow-sm">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              ...
                              <h5 className="mt-1 text-base font-black leading-snug text-slate-900">
                                {question.question}
                              </h5>
                            </div>
                          ...
                          <div className="mt-4 space-y-3">
                            <div className="rounded-2xl border border-white bg-white p-3">
                              ...
                              <p className="mt-1 text-sm font-bold text-slate-900">{question.answer}</p>
                            </div>
                            <div className="rounded-2xl border border-white bg-white p-3">
                              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">
                            ...
                            <div className="rounded-2xl border border-amber-100 bg-amber-50 p-3">
                              ...
                              <p className="mt-1 text-sm font-medium text-amber-950">{question.takeaway}</p>
                            </div>
        ...
        <section className="rounded-[2rem] border border-amber-200 bg-amber-50/80 p-5 shadow-sm md:p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              <Link
                href="/learn/sentrainer"
                className="inline-flex min-h-11 items-center gap-2 rounded-full bg-amber-600 px-5 py-3 text-sm font-black uppercase tracking-widest text-white shadow-lg shadow-amber-600/20 transition hover:bg-amber-700"
              >
                {isFrench ? "Ouvrir le quiz" : "Open the quiz"}
                <ArrowRight className="h-4 w-4" aria-hidden="true" />
              </Link>
              <Link
                href="/learn/ressources"
                className="inline-flex min-h-11 items-center gap-2 rounded-full border border-amber-200 bg-white px-5 py-3 text-sm font-black uppercase tracking-widest text-amber-900 transition hover:bg-amber-50"
              >
