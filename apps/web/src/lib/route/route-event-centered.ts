...
export function buildEventCenteredCandidates<T extends ScoredCandidate>(
  ...
  const scored = candidates.map((candidate) => {
    ...
    const proximityFactor = clamp(1 - distanceKm / ROUTE_EVENT_CENTERED_RADIUS_KM);
    const priorityAfter =
    ...
    return {
      ...candidate,
      score: priorityAfter,
      reason: `${candidate.reason} Mode événement : distance à l’événement=${distanceKm.toFixed(2)} km, proximité=${proximityFactor.toFixed(3)}, contribution=${impact.scoreContribution.toFixed(2)}.`,
