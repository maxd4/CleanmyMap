...
/** Recent post-event window with the stronger linear decay branch. */
export const ROUTE_EVENT_RECENT_WINDOW_DAYS = 16;
/** Hard horizon: post-event pressure is exactly zero at and after 8 weeks. */
export const ROUTE_EVENT_SIGNAL_HORIZON_DAYS = 56;
/** Bounded proximity window around an event; no spatial signal beyond it. */
export const ROUTE_EVENT_SPATIAL_RADIUS_KM = 2;
/** Maximum priority points contributed by the bounded event signal. */
...
export function routeEventRecencyFactor(ageDays: number): number {
  ...
  if (ageDays >= ROUTE_EVENT_SIGNAL_HORIZON_DAYS) {
    return 0;
  }
  return (
    0.5 *
...
function attendanceFactor(attendance: RouteEventAttendance): {
  ...
  const known = yes > 0 || maybe > 0 || attendance.capacityTarget !== null;

  // Missing attendance is explicit uncertainty, not zero attendance and not a
  // fabricated crowd size. The neutral 0.5 factor keeps location/recency
  // useful while making known attendance increase the bounded signal.
  return {
...
export function calculateRouteEventPressure(
  ...
  if (!isValidRouteEventCoordinatePair(event.latitude, event.longitude)) {
    return null;
  }
  ...
  if (ageDays === null || ageDays >= ROUTE_EVENT_SIGNAL_HORIZON_DAYS) {
    return null;
  }
  ...
  const pressure = clamp(
    recencyFactor * proximityFactor * attendanceMetrics.factor,
  );
