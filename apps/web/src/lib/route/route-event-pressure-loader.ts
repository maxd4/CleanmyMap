...
async function loadRouteEventSnapshot(
  ...
  const result = await supabase
    ...
    .gte("event_date", isoDateDaysFromNow(now, -ROUTE_EVENT_SIGNAL_HORIZON_DAYS))
  ...
  if (events.length === 0) return { events: [], attendanceByEventId };
  ...
  return { events: events.map(toEventRecord), attendanceByEventId };
}
...
function buildRouteEventSignalContext(
  ...
  return {
    ...
    eventsWithoutCoordinates: completedEvents.length - geolocatedCompletedEvents.length,
