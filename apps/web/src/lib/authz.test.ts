...
describe("authz helpers", () => {
  it("parses admin user ids from csv", () => {
    const ids = __authz_testables.parseAdminUserIds(" user_1, user_2 ,,user_3 ");
  ...
  it("accepts admin role from public metadata", () => {
    expect(isAdminRole({ publicMetadata: { role: "admin" }, privateMetadata: {} })).toBe(true);
  });

  it("accepts admin role from private metadata", () => {
    expect(isAdminRole({ publicMetadata: {}, privateMetadata: { role: "admin" } })).toBe(true);
  });

  it("rejects non admin role", () => {
    expect(isAdminRole({ publicMetadata: { role: "member" }, privateMetadata: { role: "viewer" } })).toBe(false);
  });

  it("extracts badge ids from metadata arrays only", () => {
    expect(__authz_testables.extractBadgeIds({ badges: ["pioneer", "mentor"] })).toEqual(["pioneer", "mentor"]);
    expect(__authz_testables.extractBadgeIds({ badges: "pioneer" })).toEqual([]);
  });

  it("maps badge ids to display badges with fallback", () => {
    const badges = __authz_testables.mapBadgeIdsToBadges(["admin", "custom_badge"]);
  ...
  it("builds actor options from first name, username and user id", () => {
    const options = __authz_testables.buildActorNameOptions("Max", "max.clean", "user_123");
    expect(options).toEqual(["Max", "max.clean", "user_123"]);
  });

  it("resolves actor name from clerk options", () => {
    const selected = __authz_testables.resolveActorNameFromClerk(["Max", "max.clean", "user_123"], "max.clean");
    const fallback = __authz_testables.resolveActorNameFromClerk(["Max", "max.clean", "user_123"], "unknown");
