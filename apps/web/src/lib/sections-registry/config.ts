import type { RubriqueCategory, RubriqueDefinition } from "./base-types";
import {
  ADMIN_GODMODE_ROUTE,
  ADMIN_ROUTE,
  DASHBOARD_ROUTE,
  EXPLORER_ROUTE,
  PILOTAGE_ROUTE,
  PROFIL_ROUTE,
  SPONSOR_PORTAL_ROUTE,
} from "@/lib/accueil-pilotage-routes";

export const RUBRIQUE_CATEGORIES = [
  { id: "pilotage", label: { fr: "Accueil & Pilotage", en: "Home & Operations" } },
  { id: "terrain", label: { fr: "Actions Terrain", en: "Field actions" } },
  {
    id: "analysis",
    label: { fr: "Analyse & Contexte", en: "Analysis & Context" },
  },
  { id: "resources", label: { fr: "Guide & Outils", en: "Guide & Tools" } },
  { id: "community", label: { fr: "Communauté", en: "Community" } },
] as const satisfies readonly RubriqueCategory[];

export const RUBRIQUE_REGISTRY = [
  {
    id: "profile",
    categoryId: "pilotage",
    spaceId: "decide",
    priority: 20,
    kind: "app-route",
    route: PROFIL_ROUTE,
    label: { fr: "Profil", en: "Profile" },
    description: { fr: "Gérer votre profil", en: "Personal dashboard alias" },
    availability: "hidden",
    implementation: "finalized",
  },
  {
    id: "explorer",
    categoryId: "resources",
    spaceId: "decide",
    priority: 18,
    kind: "app-route",
    route: EXPLORER_ROUTE,
    label: { fr: "Sommaire", en: "Summary" },
    description: {
      fr: "Accéder à toutes les rubriques du site",
      en: "Site map and direct access to all sections",
    },
    searchKeywords: { fr: ["plan du site"] },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "dashboard",
    categoryId: "pilotage",
    spaceId: "decide",
    priority: 10,
    kind: "app-route",
    route: DASHBOARD_ROUTE,
    label: { fr: "Mon espace", en: "Dashboard" },
    description: { fr: "Votre profil, vos actions et votre impact", en: "Overview, profile and monitoring" },
    searchKeywords: { fr: ["pilotage"] },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "reports",
    categoryId: "analysis",
    spaceId: "decide",
    priority: 30,
    kind: "app-route",
    route: "/reports",
    label: { fr: "Rapports d'impact", en: "Impact reports" },
    description: {
      fr: "Consulter vos résultats et exporter vos données",
      en: "Analyze your results and export your data",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "methodologie",
    categoryId: "analysis",
    spaceId: "decide",
    priority: 32,
    kind: "app-route",
    route: "/methodologie",
    label: { fr: "Méthodologie", en: "Methodology" },
    description: {
      fr: "Comprendre les calculs et les règles d'impact",
      en: "Understand the calculations and impact rules",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "admin",
    categoryId: "pilotage",
    spaceId: "supervise",
    priority: 10,
    kind: "app-route",
    route: ADMIN_ROUTE,
    label: { fr: "Administration", en: "Administration" },
    description: {
      fr: "Modérer et superviser la plateforme",
      en: "Technical back office, moderation and supervision",
    },
    searchKeywords: { fr: ["back-office"] },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "pilotage",
    categoryId: "pilotage",
    spaceId: "decide",
    priority: 14,
    kind: "app-route",
    route: PILOTAGE_ROUTE,
    label: { fr: "Pilotage", en: "Operations" },
    description: {
      fr: "Suivre les priorités et les indicateurs",
      en: "Strategic cockpit and operational arbitration",
    },
    searchKeywords: { fr: ["pilotage", "cockpit"] },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "elus",
    categoryId: "pilotage",
    spaceId: "decide",
    priority: 40,
    kind: "section",
    anonymousPresentation: "disabled",
    route: "/sections/elus",
    label: { fr: "Gouvernance", en: "Governance" },
    description: {
      fr: "Coordonner les acteurs publics et suivre les résultats",
      en: "Public coordination and territorial results",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "new",
    categoryId: "terrain",
    spaceId: "execute",
    priority: 12,
    kind: "app-route",
    route: "/actions/new",
    label: { fr: "Créer une action", en: "Create an action" },
    description: {
      fr: "Préparer une action et renseigner ses résultats.",
      en: "Prepare an action before fieldwork or complete its results after completion.",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "rejoindre-une-action",
    categoryId: "terrain",
    spaceId: "execute",
    priority: 10,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/rejoindre-une-action",
    label: { fr: "Rejoindre une action", en: "Join an action" },
    description: {
      fr: "Trouver une action de groupe à rejoindre",
      en: "Starting architecture for joining a group action",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "signalement",
    categoryId: "terrain",
    spaceId: "execute",
    priority: 14,
    kind: "app-route",
    route: "/signalement",
    label: { fr: "Signaler un déchet", en: "Report waste" },
    description: {
      fr: "Signaler l'état d'un lieu pollué ou propre",
      en: "Update the observed state of a polluted or clean place",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "map",
    categoryId: "terrain",
    spaceId: "execute",
    priority: 20,
    kind: "app-route",
    route: "/actions/map",
    label: { fr: "Carte des actions", en: "Map" },
    description: { fr: "Explorer les actions et signalements sur la carte", en: "Geo view" },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "history",
    categoryId: "terrain",
    spaceId: "supervise",
    priority: 20,
    kind: "app-route",
    route: "/actions/history",
    label: { fr: "Historique terrain", en: "Field history" },
    description: {
      fr: "Consulter les enregistrements accessibles, leur qualité et les corrections à effectuer.",
      en: "Review accessible records, their quality, and corrections to make.",
    },
    availability: "hidden",
    implementation: "finalized",
  },
  {
    id: "route",
    categoryId: "terrain",
    spaceId: "execute",
    priority: 30,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/route",
    label: { fr: "Où agir ?", en: "Where to act?" },
    description: { fr: "Trouver où agir selon l'impact local", en: "Optimize your route based on impact" },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "recycling",
    categoryId: "terrain",
    spaceId: "prepare",
    priority: 30,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/recycling",
    label: { fr: "Guide du tri", en: "Recycling guide" },
    description: { fr: "Apprendre à trier et donner une seconde vie aux déchets", en: "Waste reuse and valorization" },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "compost",
    categoryId: "terrain",
    spaceId: "prepare",
    priority: 31,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/compost",
    label: { fr: "Compostage", en: "Composting" },
    description: {
      fr: "Apprendre à composter chez soi ou dans son quartier",
      en: "Guide to composting at home or locally",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "climate",
    categoryId: "analysis",
    spaceId: "decide",
    priority: 50,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/climate",
    label: { fr: "Comprendre l'enjeu", en: "Climate context" },
    description: {
      fr: "Comprendre les enjeux climatiques et notre impact",
      en: "Planetary boundaries and the meaning of our impact",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "weather",
    categoryId: "terrain",
    spaceId: "execute",
    priority: 25,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/weather",
    label: { fr: "Organiser une action", en: "Organize an action" },
    description: {
      fr: "Consulter la météo et préparer une cleanwalk",
      en: "Check the real weather, prepare the field and organize an effective cleanwalk",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "community",
    categoryId: "community",
    spaceId: "execute",
    priority: 40,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/community",
    label: { fr: "Communauté", en: "Community" },
    description: {
      fr: "Échanger avec la communauté et trouver des partenaires",
      en: "Community coordination and partner network access",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "feedback",
    categoryId: "community",
    spaceId: "decide",
    priority: 41,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/feedback",
    label: { fr: "Idées et problèmes", en: "Ideas & issues" },
    description: {
      fr: "Partager une idée ou signaler un problème",
      en: "Share ideas and report issues",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "gamification",
    categoryId: "community",
    spaceId: "decide",
    priority: 60,
    kind: "section",
    anonymousPresentation: "disabled",
    route: "/sections/gamification",
    label: { fr: "Progression et badges", en: "Your progress" },
    description: { fr: "Suivre votre progression et vos réussites", en: "Track your evolution and badges" },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "actors",
    categoryId: "community",
    spaceId: "prepare",
    priority: 40,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/actors",
    label: { fr: "Réseau engagé", en: "Partner network" },
    description: { fr: "Trouver les acteurs engagés près de chez vous", en: "Discover locally engaged organizations" },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "network",
    categoryId: "community",
    spaceId: "prepare",
    priority: 42,
    kind: "app-route",
    route: "/partners/network",
    label: { fr: "Communauté (alias partenaires)", en: "Community (partner alias)" },
    description: {
      fr: "Accéder au réseau de partenaires",
      en: "Technical alias to the Community page",
    },
    availability: "hidden",
    implementation: "finalized",
  },
  {
    id: "annuaire",
    categoryId: "community",
    spaceId: "prepare",
    priority: 45,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/annuaire",
    label: { fr: "Annuaire des acteurs", en: "Directory" },
    description: {
      fr: "Trouver les contacts utiles dans votre zone",
      en: "Find the right contacts in your area",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "dm",
    categoryId: "community",
    spaceId: "execute",
    priority: 41,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/dm",
    label: { fr: "Messages privés", en: "Messages" },
    description: {
      fr: "Échanger directement avec les membres du réseau",
      en: "Direct exchanges with network members",
    },
    availability: "hidden",
    implementation: "finalized",
  },
  {
    id: "messagerie",
    categoryId: "community",
    spaceId: "execute",
    priority: 42,
    kind: "section",
    anonymousPresentation: "blur",
    route: "/sections/messagerie",
    label: { fr: "Groupes de discussion", en: "Discussion groups" },
    description: {
      fr: "Échanger en groupe sur des sujets locaux",
      en: "Thematic and local group discussions",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "open-data",
    categoryId: "analysis",
    spaceId: "decide",
    priority: 47,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/open-data",
    label: { fr: "Données publiques", en: "Open Data" },
    description: {
      fr: "Consulter les données publiques et les exporter",
      en: "Open data and research exports",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "funding",
    categoryId: "community",
    spaceId: "decide",
    priority: 48,
    kind: "section",
    anonymousPresentation: "visible",
    route: "/sections/funding",
    label: { fr: "Soutenir le Projet", en: "Support Us" },
    description: {
      fr: "Soutenir le projet et parrainer une zone",
      en: "Eco-patronage and zone sponsorship",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "trash-spotter",
    categoryId: "community",
    spaceId: "supervise",
    priority: 50,
    kind: "section",
    anonymousPresentation: "blur",
    route: "/sections/trash-spotter",
    label: { fr: "Suivi Trash Spotter", en: "Trash Spotter monitoring" },
    description: {
      fr: "Consulter les signalements Trash Spotter approuvés",
      en: "Review the map and approved Trash Spotter reports",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "learn-comprendre",
    categoryId: "analysis",
    spaceId: "prepare",
    priority: 6,
    kind: "app-route",
    route: "/learn/comprendre",
    label: { fr: "Vulgarisation", en: "Explanation" },
    description: {
      fr: "Comprendre les enjeux et les actions de dépollution",
      en: "Scientific context, cues and explanations",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "learn-sentrainer",
    categoryId: "resources",
    spaceId: "prepare",
    priority: 7,
    kind: "app-route",
    route: "/learn/sentrainer",
    label: { fr: "S'entraîner", en: "Practice" },
    description: {
      fr: "Tester vos connaissances avec des quiz et exercices",
      en: "Quizzes and exercises to test your knowledge",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "learn-bonnes-pratiques",
    categoryId: "resources",
    spaceId: "prepare",
    priority: 8,
    kind: "app-route",
    route: "/learn/bonnes-pratiques",
    label: {
      fr: "Bonnes pratiques",
      en: "Good practices",
    },
    description: {
      fr: "Appliquer les bonnes pratiques de tri et de compostage",
      en: "Short cues to sort well, compost and avoid litter",
    },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "sponsor",
    categoryId: "pilotage",
    spaceId: "decide",
    priority: 35,
    kind: "app-route",
    route: SPONSOR_PORTAL_ROUTE,
    label: { fr: "Portail décideurs", en: "Sponsor portal" },
    description: { fr: "Suivre l'impact et les indicateurs du territoire", en: "Impact analysis and territorial monitoring" },
    availability: "available",
    implementation: "finalized",
  },
  {
    id: "godmode",
    categoryId: "pilotage",
    spaceId: "supervise",
    priority: 5,
    kind: "app-route",
    route: ADMIN_GODMODE_ROUTE,
    label: { fr: "Administration avancée", en: "Advanced administration" },
    description: {
      fr: "Administrer les réglages avancés de la plateforme",
      en: "Hidden administration sub-section reserved for the max profile",
    },
    availability: "hidden",
    implementation: "finalized",
  },
] as const satisfies readonly RubriqueDefinition[];
