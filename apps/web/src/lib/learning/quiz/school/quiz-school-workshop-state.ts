import {
  ...
  QUIZ_SCHOOL_WORKSHOP_PRE_QUIZ_SIZE,
...
export function nextQuizSchoolWorkshopPhase(
  state: QuizSchoolWorkshopState,
  preQuestionCount = QUIZ_SCHOOL_WORKSHOP_PRE_QUIZ_SIZE,
  activityCount = 1,
  postQuestionCount = preQuestionCount,
): QuizSchoolWorkshopState {
  if (state.phase === "pre-quiz") {
    return state.questionIndex < preQuestionCount - 1
  ...
  if (state.phase === "post-quiz") {
    return state.questionIndex < postQuestionCount - 1
      ? { ...state, questionIndex: state.questionIndex + 1 }
...
export function previousQuizSchoolWorkshopPhase(
  ...
  if (state.phase === "atelier") {
    return state.activityIndex > 0
      ? { ...state, activityIndex: state.activityIndex - 1 }
      : { ...state, phase: "pre-quiz", questionIndex: Math.max(preQuestionCount - 1, 0) };
  }
  if (state.phase === "post-quiz") {
  ...
  return { ...state, phase: "post-quiz", questionIndex: Math.max(postQuestionCount - 1, 0) };
}

export function getQuizSchoolWorkshopProgress(
  state: QuizSchoolWorkshopState,
  preQuestionCount = QUIZ_SCHOOL_WORKSHOP_PRE_QUIZ_SIZE,
  postQuestionCount = preQuestionCount,
) {
  ...
  return {
    ...
    preTotal: preQuestionCount,
    postTotal: postQuestionCount,
    total: postQuestionCount,
  };
}
