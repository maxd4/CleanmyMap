  ...
  return `
    ...
    <div style="display: flex; flex-direction: column; gap: 2px;">
      ${MASTER_PACK_CHAPTERS.filter(c => c.id !== "sommaire").map((chapter, i) => `
        <a href="#chapter-${escapeHtml(chapter.id)}" style="display: flex; justify-content: space-between; align-items: center; padding: 18px 0; border-bottom: 1px solid #f1f5f9; text-decoration: none; color: #1e293b;">
          <div style="display: flex; flex-direction: column; gap: 4px;">
            <span style="font-weight: 700; font-size: 16px; font-family: 'Outfit';">${i + 1}. ${escapeHtml(chapter.title)}</span>
            <span style="font-size: 12px; color: #64748b;">${escapeHtml(chapter.subtitle)}</span>
          </div>
