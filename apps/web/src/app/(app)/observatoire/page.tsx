...
export default async function ObservatoirePage() {
 ...
 return (
 <div className={`min-h-screen p-6 sm:p-12 space-y-12 transition-colors duration-500 ${isSober ? 'bg-white cmm-text-primary' : 'bg-slate-950 text-slate-100'}`}>
 ...
 <p className={`cmm-text-caption font-bold uppercase tracking-widest ${isSober ? 'cmm-text-muted' : 'cmm-text-muted'}`}>{kpi.label}</p>
 <p className={`text-4xl font-bold transition-colors ${isSober ? 'cmm-text-primary' : 'text-white group-hover:text-emerald-400'}`}>{kpi.value}</p>
 </div>
 ))}
 </div>
 ...
 <Link 
 href="/sign-in"
 className={`px-6 py-3 rounded-xl font-bold cmm-text-small uppercase tracking-wider transition-all active:scale-95 ${isSober ? 'bg-slate-900 text-white hover:bg-slate-800' : 'bg-white text-slate-950 hover:bg-emerald-400'}`}
 >
