...
describe("gamification points analytics route", () => {
  it("uses named scopes and the Supabase analytics helper", () => {
    ...
    expect(source).toContain("loadGamificationPointsAnalytics");
    expect(source).not.toContain('.from("points_ledger")');
  });
});
