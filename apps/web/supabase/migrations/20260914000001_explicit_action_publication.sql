-- Explicit publication is separate from moderation and from the derived
-- temporal property "future". A pre-action is private until this is set.
alter table public.actions
  add column if not exists published_at timestamptz;

create index if not exists idx_actions_published_future
  on public.actions(action_date, event_start_time)
  where published_at is not null and action_phase = 'pre_action';

-- The owner may still read private drafts directly; public roles may only read
-- approved completed observations or explicitly published future pre-actions.
      ...
      or (
        ...
        and (
          ...
          or (
            ...
            and event_start_time > (current_time at time zone 'Europe/Paris')::time
          )
        )
      )
    )
...
-- Keep the public map projection closed over the explicit publication marker.
-- A published pre-action is projected only while its local Paris start is in
-- the future; status remains the persisted moderation status internally.
