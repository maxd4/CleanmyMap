-- Harden the existing administrative-requirements transition without editing
-- the already-applied migration. The route performs AuthN/AuthZ; this RPC is
-- executable only through the server service-role context.
...
begin
  ...
  -- The row lock serializes retries and concurrent validations for this action.
  select *
  into v_action
  from public.actions
  ...
  -- Update only the protected JSONB path; all other preparation fields remain.
  ...
  -- This write is in the same transaction as the locked update. A failure
  -- rolls back both the state transition and this audit entry.
