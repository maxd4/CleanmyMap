-- XP is the only CURRENT progression unit. Point tables remain compatibility data.

drop function if exists public.load_gamification_funnel_counts();

create function public.load_gamification_funnel_counts()
returns table (
  total_users integer,
...
as $$
  select
    coalesce((select count(*)::integer from public.progression_profiles), 0) as total_users,
    coalesce((select count(*)::integer from public.progression_profiles where xp_validated > 0), 0) as users_with_xp,
    coalesce((select count(*)::integer from public.progression_profiles where current_level > 1), 0) as users_with_badges,
    coalesce((select count(*)::integer from public.progression_profiles where xp_total >= 500), 0) as users_high_activity;
$$;
...
drop function if exists public.load_gamification_user_counters(text);

create function public.load_gamification_user_counters(p_user_id text)
returns table (
