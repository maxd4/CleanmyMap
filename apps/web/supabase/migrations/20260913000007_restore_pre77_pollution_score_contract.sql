-- Restore the last pollution-score contract versioned before 77e72b0b.
-- The repository contains no canonical decision authorizing volunteer-hour
-- normalization. The active contract therefore remains quantity per volunteer;
-- NULL metrics stay absent and never become zero. Department rows and counts
-- remain part of the same V2 source.
...
as $$
  with eligible_actions as (
    select
      ...
      a.waste_kg,
      a.cigarette_butts,
      a.volunteers_count::numeric as volunteers_count
  ...
  department_references as (
    select
      ...
      max(waste_kg::numeric / volunteers_count)
        filter (where waste_kg is not null and waste_kg >= 0)
        as waste_per_volunteer,
      max(cigarette_butts::numeric / volunteers_count)
  ...
  select
    ...
    max(waste_kg::numeric / volunteers_count)
      filter (where waste_kg is not null and waste_kg >= 0) as waste_per_volunteer,
    max(cigarette_butts::numeric / volunteers_count)
      filter (where cigarette_butts is not null and cigarette_butts >= 0)
      as butts_per_volunteer,
    count(*) filter (where waste_kg is not null and waste_kg >= 0)::integer
