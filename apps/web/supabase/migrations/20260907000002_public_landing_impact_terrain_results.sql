-- IMPACT-2026-01: extend the bounded public landing aggregate with the
-- qualified cigarette-butt distribution used by the methodology page.
-- The existing columns and public scope remain unchanged and unqualified
-- historical butts are deliberately kept outside the condition buckets.
...
with source_actions as (
  ...
  select
    megots_condition as condition,
    sum(cigarette_butts)::bigint as count
  from classified_actions
  where megots_condition is not null
    and cigarette_butts > 0
  group by megots_condition
)
select
  ...
  coalesce(
    (
      select jsonb_agg(
        jsonb_build_object('condition', condition, 'count', count)
        order by condition
      )
      from butt_conditions
    ),
    '[]'::jsonb
  ) as butts_by_condition
from classified_actions;
$$;
