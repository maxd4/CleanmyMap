-- Keep the public map projection independent from caller-controlled status
-- filters. The parameter remains in the signature for client compatibility.
...
for select
using (status in ('validated', 'cleaned'));
