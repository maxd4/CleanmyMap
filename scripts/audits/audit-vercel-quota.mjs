...
const baselinePath = path.join(repoRoot, "scripts", "audits", "vercel-quota-audit-baseline.json");
...
if (!baseline) {
  ...
  console.error(
    `Génère-la avec: node ${toRepoRelative(path.join(repoRoot, "scripts", "audits", "audit-vercel-quota.mjs"))} --write-baseline`,
  );
  process.exit(1);
}
