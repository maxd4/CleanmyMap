...
if ($encodingCheckExit -ne 0) {
    ...
    Invoke-Step { python maintenance/python/scripts/normalize_utf8.py --root . --write --max-report 20 } "normalize_utf8_write"
    Invoke-Step { python maintenance/python/scripts/normalize_utf8.py --root . --check --max-report 20 } "normalize_utf8_recheck"
}

if ($Scope -eq "full") {
    Write-Host "Running Python static compilation (full tracked set)..."
    $pythonFiles = @(git ls-files 'maintenance/python/src/*.py' 'maintenance/python/scripts/*.py')
} else {
    $pythonFiles = @($changedFiles | Where-Object { $_ -like 'maintenance/python/src/*.py' -or $_ -like 'maintenance/python/scripts/*.py' })
...
Invoke-Step { python maintenance/python/scripts/ci_cleanup.py --root . --check } "ci_cleanup"

Write-Host "Running runtime SQLite tracking guardrail..."
Invoke-Step { python maintenance/python/scripts/check_runtime_db_tracking.py --root . } "runtime_db_tracking"
...
if ($Scope -eq "full") {
    ...
    $changedTestFiles = @($changedFiles | Where-Object { $_ -like 'maintenance/python/tests/*.py' })
    if ($changedTestFiles.Count -gt 0) {
        ...
        Invoke-Step { pytest -q maintenance/python/tests/test_cleanup_audit.py maintenance/python/tests/test_ci_cleanup_cli.py maintenance/python/tests/test_normalize_utf8.py } "pytest_smoke"
    }
}
